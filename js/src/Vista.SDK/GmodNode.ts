...
export class GmodNode {
  ...
  public static createFromDto(dto: GmodNodeDto) {
    ...
    return new GmodNode(
      ...
      code,
      {
        ...
        normalAssignmentNames: new Map<string, string>(Object.entries(normalAssignmentNames ?? {})),
      },
      location
    );
  }
  ...
  public withLocation(_location: string) {
    return new GmodNode(this._id, this.code, this.metadata, _location, this.parents, this.children);
  }

  public withoutLocation(): GmodNode {
    return new GmodNode(this._id, this.code, this.metadata, undefined, this.parents, this.children);
  }
  ...
  public isChild(node: GmodNode | string): boolean {
    return typeof node === 'string' ? this.isChildFromCode(node) : this.isChildFromCode(node.code);
  }
  ...
  public addChild(child: GmodNode) {
    ...
    this._children = this._children.sort((a, b) => naturalSort(this.getSortString(a), this.getSortString(b)));
  }

  public addParent(parent: GmodNode) {
    this._parents.push(parent);
    this._parents = this._parents.sort((a, b) => naturalSort(this.getSortString(a), this.getSortString(b)));
  }

  private getSortString(node: GmodNode) {
