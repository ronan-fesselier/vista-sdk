import { CodebookName, Codebooks, Gmod, GmodPath, LocalId, MetadataTag } from '.';
import { LocalIdErrorBuilder } from './internal/LocalIdErrorBuilder';
import { LocalIdItems } from './LocalId.Items';
import { LocalIdParser } from './LocalId.Parser';
...
export class LocalIdBuilder {
  ...
  public get isEmpty(): boolean {
    return !this._items.primaryItem && !this._items.secondaryItem && this.isEmptyMetadata;
  }
  ...
  public validate(errorBuilder = new LocalIdErrorBuilder()): LocalIdErrorBuilder {
    ...
    if (this.isEmptyMetadata) {
      errorBuilder.push({ type: ParsingState.EmptyState, message: 'Missing any metadata tag' });
    }
  ...
  public equals(other?: LocalIdBuilder): boolean {
    ...
    if (this.visVersion !== other.visVersion) throw new Error('Cant compare local IDs from different VisVersions');
    return !!(
  ...
  public toString(builder: string[] = []): string {
    if (!this.visVersion) throw new Error('No VisVersion configured on LocalId');
  ...
  public withMetadataTag(metadataTag: MetadataTag): LocalIdBuilder {
    switch (metadataTag.name) {
      ...
      default:
        throw new Error(`Unknown or invalid metadata tag name: ${metadataTag.name}`);
    }
  }
  ...
  public static tryParse(
    ...
    return LocalIdParser.tryParse(localIdStr, gmod, codebooks, errorBuilder);
  }
}
