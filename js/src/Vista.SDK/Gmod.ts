...
export class Gmod {
  ...
  public constructor(visVersion: VisVersion, dto: GmodDto) {
    ...
    for (const relation of dto.relations) {
      const parentCode = relation.parent;
      const childCode = relation.child;

      const parentNode = this._nodeMap.get(parentCode);
      const childNode = this._nodeMap.get(childCode);
      if (!parentNode) throw new Error('Couldnt find parent node');
      if (!childNode) throw new Error('Couldnt find child node');
  ...
  public static isLeafNode(values: GmodTuple | GmodNodeMetadata): boolean {
    ...
    for (const leafType of this.leafTypes) {
      if (leafType.category === category && leafType.type === type) return true;
    }
  ...
  public static isProductSelection(metadata: GmodNodeMetadata) {
    return metadata.category.includes('PRODUCT') && metadata.type.includes('SELECTION');
  }
  ...
  public static isProductTypeAssignment(parent?: GmodNode, child?: GmodNode) {
    ...
    if (child.metadata.category !== 'PRODUCT' || child.metadata.type !== 'TYPE') return false;
    return true;
  }

  public static isProductSelectionAssignment(parent?: GmodNode, child?: GmodNode) {
    if (!parent || !child) return false;
    if (!parent.metadata.category.includes('FUNCTION')) return false;
    if (!child.metadata.category.includes('PRODUCT') || child.metadata.type !== 'SELECTION') return false;
    return true;
  }
  ...
  public traverse<T>(
    ...
    if (!state) {
      return this.traverseFromNodeWithState(handler as TraversalHandler, rootNode, (parents, node, handler) =>
        handler(parents, node)
      );
    }

    return this.traverseFromNodeWithState(state, rootNode, handler as TraversalHandlerWithState<T>);
  }

  public traverseFromNodeWithState<T>(state: T, rootNode: GmodNode, handler: TraversalHandlerWithState<T>): boolean {
    const context: TraversalContext<T> = { parents: new Parents(), handler, state };
    return this.traverseNode<T>(context, rootNode) === TraversalHandlerResult.Continue;
  }

  private traverseNode<T>(context: TraversalContext<T>, node: GmodNode): TraversalHandlerResult {
    if (context.parents.has(node)) return TraversalHandlerResult.Continue;

    let result = context.handler(context.parents.asList, node, context.state);

    if (result === TraversalHandlerResult.Stop || result === TraversalHandlerResult.SkipSubtree) return result;
