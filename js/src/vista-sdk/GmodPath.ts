...
export class GmodPath {
  ...
  public constructor(parents: GmodNode[], node: GmodNode, skipVerify = false) {
    if (!skipVerify) {
      if (parents.length === 0 && node.code !== 'VE')
        throw new Error(`Invalid gmod path - no parents, and ${node.code} is not the root of gmod`);
      if (parents.length > 0 && parents[0].code !== 'VE')
        throw new Error(`Invalid gmod path - first parent should be root of gmod (VE), but was ${parents[0].code}`);

      for (let i = 0; i < parents.length; i++) {
        const parent = parents[i];
        const nextIndex = i + 1;
        const child = nextIndex < parents.length ? parents[nextIndex] : node;
        if (!parent.isChild(child)) throw new Error(`Invalid gmod path - ${child.code} not child of ${parent.code}`);
      }
    }
  ...
  public getNode(depth: number): GmodNode {
    if (depth < 0 || depth > this.parents.length) throw new RangeError('Depth is out of bounds');
  ...
  public toString(): string {
    ...
    for (const parent of this.parents) {
      if (!parent.isLeafNode) continue;
      s.push(parent.toString());
    }

    return s.concat(this.node.code).join('/');
  }
  ...
  public getCommonNames(): { depth: number; name: string }[] {
    ...
    for (let depth = 0; depth < fullPath.length; depth++) {
      ...
      if (!(node.isLeafNode || isTarget) || !node.isFunctionNode) continue;
      ...
      if (normalAssignmentNames.size !== 0) {
        ...
        for (let i = this.parents.length - 1; i >= depth; i--) {
          const assignment = normalAssignmentNames.get(this.parents[i].code);
  ...
  public static tryParse(item: string | undefined, gmod: Gmod): GmodPath | undefined {
    ...
    if (!toFind) throw new Error(ERROR_IDENTIFIER + 'Invalid queue operation - Shift empty array');
    ...
    gmod.traverse(
      (parents, current, context) => {
        ...
        if (!found && Gmod.isLeafNode(current.metadata)) return TraversalHandlerResult.SkipSubtree;
        ...
        const endNode = toFind.location ? current.withLocation(toFind.location) : current;

        let startNode =
          pathParents.length > 0 && pathParents[0].parents.length === 1
  ...
  public static tryParseFromFullPath(item: string, gmod: Gmod): GmodPath | undefined {
    if (!item || !gmod) return;
    if (isNullOrWhiteSpace(item)) return;
  ...
  public isValid(): boolean {
    ...
    return this.isValidLocation(this.node.location) && !this.parents.find(p => !this.isValidLocation(p.location));
  }
