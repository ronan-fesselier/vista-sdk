...
impl GmodPathQueryBuilder {
    pub(crate) fn from_ptr<'a>(
        ...
        // SAFETY: ptr is non-null and `GmodPathQueryBuilder` is `#[repr(transparent)]` over the FFI type.
...
impl Drop for OwnedGmodPathQueryBuilder {
    fn drop(&mut self) {
        // SAFETY: self.0 is owned by this `OwnedGmodPathQueryBuilder` and freed exactly once.
