...
impl GmodPathQueryBuilderRef {
    pub(crate) fn from_ptr<'a>(
        ...
        // SAFETY: ptr is non-null and `GmodPathQueryBuilderRef` is `#[repr(transparent)]` over the FFI type.
...
pub struct GmodPathQueryBuilder(pub(crate) *mut ffi::dnv_vista_sdk_gmod_path_query_builder_t);
...
impl std::ops::Deref for GmodPathQueryBuilder {
    ...
    fn deref(&self) -> &GmodPathQueryBuilderRef {
        GmodPathQueryBuilderRef::from_ptr(self.0)
    }
}
