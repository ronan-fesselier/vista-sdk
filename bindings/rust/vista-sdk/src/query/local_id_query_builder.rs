...
impl LocalIdQueryBuilder {
    pub(crate) fn from_ptr<'a>(
        ...
        // SAFETY: ptr is non-null and `LocalIdQueryBuilder` is `#[repr(transparent)]` over the FFI type.
    ...
    pub fn tags_builder(&self) -> Option<&MetadataTagsQueryBuilder> {
        // SAFETY: self is non-null and valid for the lifetime of `&self`.
        let ptr =
        ...
        if ptr.is_null() {
            ...
            Some(MetadataTagsQueryBuilder::from_ptr(ptr))
        }
    }
...
impl Drop for OwnedLocalIdQueryBuilder {
    fn drop(&mut self) {
        // SAFETY: self.0 is owned by this `OwnedLocalIdQueryBuilder` and freed exactly once.
