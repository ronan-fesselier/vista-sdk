...
impl ShipId {
    ...
    pub fn from_other_id(other_id: &str) -> Result<Self, VistaError> {
        let c_str = CString::new(other_id).map_err(|_| VistaError {
            kind: ErrorKind::InvalidArgument,
            message: "other_id contains a NUL byte".to_string(),
        })?;
        ...
        if ptr.is_null() {
            ...
            // SAFETY: ptr is owned here (used only to validate) and freed exactly once.
    ...
    pub fn from_string(value: &str) -> Option<Self> {
        ...
        // SAFETY: ptr is non-null and valid for the extent of this block.
        let result = if unsafe { ffi::dnv_vista_sdk_ship_id_is_imo_number(ptr) } != 0 {
            // SAFETY: ptr is non-null. Returns an owned pointer.
            let imo_ptr = unsafe { ffi::dnv_vista_sdk_ship_id_imo_number(ptr) };
            // SAFETY: imo_ptr is non-null and owned until freed below.
            let v = unsafe { imo_ffi::dnv_vista_sdk_imo_number_value(imo_ptr) };
            // SAFETY: imo_ptr was allocated by the library and is freed exactly once.
            unsafe { imo_ffi::dnv_vista_sdk_imo_number_free(imo_ptr) };
            ShipId::Imo(ImoNumber(v as u32))
        } else {
            // SAFETY: ptr is non-null. Returns a non-null pointer owned by `ptr`.
            let raw = unsafe { ffi::dnv_vista_sdk_ship_id_other_id(ptr) };
            // SAFETY: raw is non-null and owned by `ptr` for its lifetime.
    ...
    pub(crate) fn from_ffi_ptr(ptr: *const dnv_vista_sdk_ship_id_t) -> Self {
        // SAFETY: ptr is non-null and valid for the extent of this block (borrowed from the caller).
        ...
        F: FnOnce(*const dnv_vista_sdk_ship_id_t) -> R,
    {
        let ptr = match self {
            ShipId::Imo(imo) => {
                ...
                // SAFETY: imo_ptr is non-null. Returns an owned pointer.
