...
pub enum DateTimeFormat {
    /// ISO 8601 with second precision and `Z` suffix (e.g. `2026-07-14T14:30:45Z`).
    Iso8601 = 0,
    /// ISO 8601 with full 100-nanosecond precision (7 fractional digits).
    Iso8601Precise,
    /// ISO 8601 with fractional seconds, trailing zeros trimmed.
    Iso8601PreciseTrimmed,
    /// ISO 8601 with millisecond precision (3 fractional digits).
    Iso8601Millis,
    /// ISO 8601 with microsecond precision (6 fractional digits).
    Iso8601Micros,
    /// ISO 8601 extended format with explicit `+00:00` offset instead of `Z`.
    Iso8601Extended,
    /// ISO 8601 basic (compact) format without separators (e.g. `20260714T143045Z`).
    Iso8601Basic,
    /// Date-only ISO 8601 (e.g. `2026-07-14`).
    Iso8601Date,
    /// Time-only ISO 8601 (e.g. `T14:30:45Z`).
    Iso8601Time,
...
impl DateTime {
    /// Converts to a `std::time::SystemTime` (UTC), preserving 100-nanosecond tick precision.
    pub fn to_system_time(self) -> std::time::SystemTime {
        let epoch_ticks = self.ticks() - Self::epoch().ticks();
        let nanos = epoch_ticks * 100;
        if nanos >= 0 {
    ...
    /// Constructs a `DateTime` from a `std::time::SystemTime`, preserving 100-nanosecond tick precision.
    ///
    /// Times before the Unix epoch are clamped to [`DateTime::epoch`].
    pub fn from_system_time(st: std::time::SystemTime) -> Self {
        match st.duration_since(std::time::UNIX_EPOCH) {
            Ok(d) => Self::epoch() + TimeSpan::from_ticks(d.as_nanos() as i64 / 100),
            Err(_) => Self::epoch(),
        }
    }
}
...
mod discriminant_sync {
    ...
    #[test]
    fn date_time_format_discriminants_match_ffi() {
        assert_eq!(
            DateTimeFormat::Iso8601 as u32,
            ffi::dnv_vista_sdk_date_time_format_t::Iso8601 as u32
        );
        assert_eq!(
            DateTimeFormat::Iso8601Precise as u32,
            ffi::dnv_vista_sdk_date_time_format_t::Iso8601Precise as u32
        );
        assert_eq!(
            DateTimeFormat::Iso8601PreciseTrimmed as u32,
            ffi::dnv_vista_sdk_date_time_format_t::Iso8601PreciseTrimmed as u32
        );
        assert_eq!(
            DateTimeFormat::Iso8601Millis as u32,
            ffi::dnv_vista_sdk_date_time_format_t::Iso8601Millis as u32
        );
        assert_eq!(
            DateTimeFormat::Iso8601Micros as u32,
            ffi::dnv_vista_sdk_date_time_format_t::Iso8601Micros as u32
        );
        assert_eq!(
            DateTimeFormat::Iso8601Extended as u32,
            ffi::dnv_vista_sdk_date_time_format_t::Iso8601Extended as u32
        );
        assert_eq!(
            DateTimeFormat::Iso8601Basic as u32,
            ffi::dnv_vista_sdk_date_time_format_t::Iso8601Basic as u32
        );
        assert_eq!(
            DateTimeFormat::Iso8601Date as u32,
            ffi::dnv_vista_sdk_date_time_format_t::Iso8601Date as u32
        );
        assert_eq!(
            DateTimeFormat::Iso8601Time as u32,
            ffi::dnv_vista_sdk_date_time_format_t::Iso8601Time as u32
        );
        assert_eq!(
            DateTimeFormat::UnixSeconds as u32,
            ffi::dnv_vista_sdk_date_time_format_t::UnixSeconds as u32
        );
        assert_eq!(
            DateTimeFormat::UnixMilliseconds as u32,
            ffi::dnv_vista_sdk_date_time_format_t::UnixMilliseconds as u32
        );
    }
}
