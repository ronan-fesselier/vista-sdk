...
impl DateTimeOffset {
    ...
    pub fn new(date_time: DateTime, offset: TimeSpan) -> Self {
        // SAFETY: no pointer arguments. date_time.into_ffi()/offset.into_ffi() are plain structs passed by value.
    ...
    pub fn from_date_time(date_time: DateTime) -> Self {
        // SAFETY: no pointer arguments. date_time.into_ffi() is a plain struct passed by value.
        Self::from_ffi(unsafe {
    ...
    pub fn from_ticks(ticks: i64, offset: TimeSpan) -> Self {
        // SAFETY: no pointer arguments. offset.into_ffi() is a plain struct passed by value.
        Self::from_ffi(unsafe {
    ...
    pub fn to_offset(self, new_offset: TimeSpan) -> Self {
        // SAFETY: no pointer arguments. self.into_ffi()/new_offset.into_ffi() are plain structs passed by value.
        Self::from_ffi(unsafe {
    ...
    pub fn to_string_fmt(self, format: DateTimeFormat) -> String {
        use crate::ffi::types::date_time::dnv_vista_sdk_date_time_format_t as FfiFormat;
        let ffi_format = match format {
            DateTimeFormat::Iso8601 => FfiFormat::Iso8601,
            DateTimeFormat::Iso8601Precise => FfiFormat::Iso8601Precise,
            DateTimeFormat::Iso8601PreciseTrimmed => FfiFormat::Iso8601PreciseTrimmed,
            DateTimeFormat::Iso8601Millis => FfiFormat::Iso8601Millis,
            DateTimeFormat::Iso8601Micros => FfiFormat::Iso8601Micros,
            DateTimeFormat::Iso8601Extended => FfiFormat::Iso8601Extended,
            DateTimeFormat::Iso8601Basic => FfiFormat::Iso8601Basic,
            DateTimeFormat::Iso8601Date => FfiFormat::Iso8601Date,
            DateTimeFormat::Iso8601Time => FfiFormat::Iso8601Time,
            DateTimeFormat::UnixSeconds => FfiFormat::UnixSeconds,
            DateTimeFormat::UnixMilliseconds => FfiFormat::UnixMilliseconds,
        };
        // SAFETY: self.into_ffi() is a plain struct passed by value. ffi_format is a valid discriminant.
        let ptr =
            unsafe { ffi::dnv_vista_sdk_date_time_offset_to_string(self.into_ffi(), ffi_format) };
        if ptr.is_null() {
            return String::new();
        }
...
impl DateTimeOffset {
    /// Converts to a `std::time::SystemTime` (UTC instant, offset discarded), preserving
    /// 100-nanosecond tick precision.
    pub fn to_system_time(self) -> std::time::SystemTime {
        let epoch_ticks = self.utc_ticks() - Self::epoch().utc_ticks();
    ...
    /// Constructs a `DateTimeOffset` (UTC, zero offset) from a `std::time::SystemTime`,
    /// preserving 100-nanosecond tick precision.
    ///
    /// Times before the Unix epoch are clamped to [`DateTimeOffset::epoch`].
    pub fn from_system_time(st: std::time::SystemTime) -> Self {
        match st.duration_since(std::time::UNIX_EPOCH) {
            Ok(d) => {
                Self::epoch()
                    + crate::types::time_span::TimeSpan::from_ticks(d.as_nanos() as i64 / 100)
            }
            Err(_) => Self::epoch(),
        }
    }
