...
impl LocalIdBuilder {
    pub(crate) fn from_ptr<'a>(ptr: *const ffi::dnv_vista_sdk_local_id_builder_t) -> &'a Self {
        ...
        // SAFETY: ptr is non-null and `LocalIdBuilder` is `#[repr(transparent)]` over the FFI type.
...
impl Drop for OwnedLocalIdBuilder {
    fn drop(&mut self) {
        // SAFETY: self.0 is owned by this `OwnedLocalIdBuilder` and freed exactly once.
