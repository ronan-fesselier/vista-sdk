...
impl UniversalIdBuilderRef {
    ...
    pub(crate) unsafe fn from_ptr<'a>(
        ptr: *const ffi::dnv_vista_sdk_universal_id_builder_t,
    ) -> &'a Self {
        // SAFETY: caller guarantees ptr is non-null and valid. `UniversalIdBuilderRef` is `#[repr(transparent)]`.
...
impl std::ops::Deref for UniversalIdBuilder {
    ...
    fn deref(&self) -> &UniversalIdBuilderRef {
        ...
        unsafe { UniversalIdBuilderRef::from_ptr(self.0.as_ptr()) }
    }
}
...
impl UniversalIdBuilder {
    ...
    pub fn from_string_with_errors(universal_id_str: &str) -> (Option<UniversalId>, ParsingErrors) {
        let c_str = match CString::new(universal_id_str) {
            Ok(c) => c,
...
// SAFETY: UniversalIdBuilderRef is a borrowed view (`repr(transparent)`). Sync only, not Send.
unsafe impl Sync for UniversalIdBuilderRef {}

// SAFETY: UniversalIdBuilder owns its heap-allocated C++ object exclusively.
unsafe impl Send for UniversalIdBuilder {}
unsafe impl Sync for UniversalIdBuilder {}
