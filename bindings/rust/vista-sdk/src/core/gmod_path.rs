...
///
/// Borrowed - obtained via accessors like [`crate::core::local_id::LocalIdRef::primary_item`].
/// See [`GmodPath`] for the owned variant returned by parsing/building functions.
#[repr(transparent)]
pub struct GmodPathRef(ffi::dnv_vista_sdk_gmod_path_t);

impl GmodPathRef {
    /// # Safety
    /// `ptr` must be non-null and valid for `'a`.
    pub(crate) unsafe fn from_ptr<'a>(ptr: *const ffi::dnv_vista_sdk_gmod_path_t) -> &'a Self {
        // SAFETY: caller guarantees ptr is non-null and valid. `GmodPathRef` is `#[repr(transparent)]`.
        unsafe { &*(ptr as *const GmodPathRef) }
    }
    ...
    pub fn node(&self) -> &GmodNodeRef {
        // SAFETY: self is non-null and valid for the lifetime of `&self`.
        let ptr = unsafe { ffi::dnv_vista_sdk_gmod_path_node(self.as_ffi_ptr()) };
        // SAFETY: ptr is non-null (node always exists for a valid path) and valid for `self`'s lifetime.
        unsafe { GmodNodeRef::from_ptr(ptr) }
    }
    ...
    pub fn at(&self, index: usize) -> Option<&GmodNodeRef> {
        // SAFETY: self is non-null and valid for the lifetime of `&self`.
    ...
    pub fn iter(&self) -> impl Iterator<Item = &GmodNodeRef> {
    ...
    pub fn without_locations(&self) -> GmodPath {
        ...
        GmodPath(
            std::ptr::NonNull::new(ptr)
                .expect("dnv_vista_sdk_gmod_path_without_locations returned NULL"),
        )
    }
...
impl PartialEq for GmodPathRef {
...
impl Eq for GmodPathRef {}

impl std::fmt::Display for GmodPathRef {
...
impl std::fmt::Debug for GmodPathRef {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "GmodPath({})", self)
    }
}

/// An owned, independently-released [`GmodPathRef`].
///
/// Returned by parsing functions (e.g. [`GmodPath::from_short_path`]) and other
/// APIs that hand back a new path value rather than a reference into existing state.
pub struct GmodPath(pub(crate) std::ptr::NonNull<ffi::dnv_vista_sdk_gmod_path_t>);

impl Drop for GmodPath {
    fn drop(&mut self) {
        // SAFETY: self.0 is owned by this `GmodPath` and freed exactly once.
        unsafe { ffi::dnv_vista_sdk_gmod_path_free(self.0.as_ptr()) }
    }
}
...
impl AsRef<GmodPathRef> for GmodPath {
    fn as_ref(&self) -> &GmodPathRef {
        self
    }
}

impl std::ops::Deref for GmodPath {
    type Target = GmodPathRef;

    fn deref(&self) -> &GmodPathRef {
        // SAFETY: self.0 is NonNull, so as_ptr() is always non-null and valid for `self`'s lifetime.
        unsafe { GmodPathRef::from_ptr(self.0.as_ptr()) }
    }
}

impl GmodPath {
    pub(crate) fn from_owned_ptr(ptr: *mut ffi::dnv_vista_sdk_gmod_path_t) -> Self {
        GmodPath(std::ptr::NonNull::new(ptr).expect("gmod_path pointer must not be NULL"))
    }
    ...
    pub fn from_short_path_version(item: &str, vis_version: VisVersion) -> Option<Self> {
        ...
        // SAFETY: c_item and c_version are valid NUL-terminated C strings. Returns an owned pointer or NULL.
        let ptr = unsafe {
        ...
        std::ptr::NonNull::new(ptr).map(GmodPath)
    }
    ...
    pub fn from_short_path(item: &str, gmod: &Gmod, locations: &Locations) -> Option<Self> {
        let c_item = CString::new(item).ok()?;
        // SAFETY: c_item is a valid NUL-terminated C string, and gmod/locations are non-null.
        let ptr = unsafe {
    ...
    pub fn from_short_path_with_errors(
        ...
        // SAFETY: c_item is a valid NUL-terminated C string, gmod/locations are non-null, and
        // `errors_ptr` is a valid writable pointer to a local that the callee always sets.
        let ptr = unsafe {
    ...
    pub fn from_full_path(full_path_str: &str, gmod: &Gmod, locations: &Locations) -> Option<Self> {
        ...
        // SAFETY: c_str is a valid NUL-terminated C string, and gmod/locations are non-null.
        let ptr = unsafe {
    ...
    pub fn from_full_path_with_errors(
        ...
        let mut errors_ptr = ptr::null_mut();
        // SAFETY: c_str is a valid NUL-terminated C string, gmod/locations are non-null, and
        // `errors_ptr` is a valid writable pointer to a local that the callee always sets.
        let ptr = unsafe {
...
// SAFETY: GmodPathRef is a borrowed view (`repr(transparent)`). Sync only, not Send.
unsafe impl Sync for GmodPathRef {}

// SAFETY: GmodPath owns its heap-allocated C++ object exclusively.
unsafe impl Send for GmodPath {}
unsafe impl Sync for GmodPath {}
