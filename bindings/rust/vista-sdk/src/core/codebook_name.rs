...
pub enum CodebookName {
    ...
    Quantity = 1,
    /// Substance or medium the sensor measures.
    Content,
    /// Calculation method applied to the measured value.
    Calculation,
    /// Operating state of the equipment.
    State,
    /// Control command issued to equipment.
    Command,
    /// Equipment or component type.
    Type,
    /// Functional service the equipment belongs to.
    FunctionalServices,
    /// Maintenance category.
    MaintenanceCategory,
    /// Activity type performed on the equipment.
    ActivityType,
    /// Physical position on the vessel.
    Position,
    /// Additional detail qualifier.
    Detail,
}
...
pub mod codebook_names {
    ...
    pub fn from_prefix(prefix: &str) -> Option<CodebookName> {
        ...
        // SAFETY: c_prefix is a valid NUL-terminated C string, and `out` is a valid
        // writable pointer to a local.
        let ok =
    ...
    pub fn to_prefix(name: CodebookName) -> &'static str {
        // SAFETY: `name` maps to a valid discriminant (checked below by discriminant_sync).
