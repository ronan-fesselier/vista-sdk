...
impl LocationBuilder {
    ...
    pub fn with_code(self, code: char) -> Result<Self, VistaError> {
        // SAFETY: self.0 is non-null and consumed by this call; returns an owned pointer or NULL.
        let ptr = unsafe { ffi::dnv_vista_sdk_location_builder_with_code(self.0, code as c_char) };
        if ptr.is_null() {
            Err(last_error())
        } else {
            Ok(LocationBuilder(ptr))
    ...
    pub fn with_side(self, side: char) -> Result<Self, VistaError> {
        // SAFETY: self.0 is non-null and consumed by this call; returns an owned pointer or NULL.
        let ptr = unsafe { ffi::dnv_vista_sdk_location_builder_with_side(self.0, side as c_char) };
        if ptr.is_null() {
            Err(last_error())
        } else {
            Ok(LocationBuilder(ptr))
    ...
    pub fn with_vertical(self, vertical: char) -> Result<Self, VistaError> {
        // SAFETY: self.0 is non-null and consumed by this call; returns an owned pointer or NULL.
        let ptr = unsafe {
            ffi::dnv_vista_sdk_location_builder_with_vertical(self.0, vertical as c_char)
        };
        if ptr.is_null() {
            Err(last_error())
        } else {
    ...
    pub fn with_transverse(self, transverse: char) -> Result<Self, VistaError> {
        // SAFETY: self.0 is non-null and consumed by this call; returns an owned pointer or NULL.
        let ptr = unsafe {
            ffi::dnv_vista_sdk_location_builder_with_transverse(self.0, transverse as c_char)
        };
        if ptr.is_null() {
            Err(last_error())
        } else {
    ...
    pub fn with_longitudinal(self, longitudinal: char) -> Result<Self, VistaError> {
        // SAFETY: self.0 is non-null and consumed by this call; returns an owned pointer or NULL.
        let ptr = unsafe {
            ffi::dnv_vista_sdk_location_builder_with_longitudinal(self.0, longitudinal as c_char)
        };
        if ptr.is_null() {
            Err(last_error())
        } else {
