...
impl Locations {
    ...
    pub(crate) unsafe fn from_ptr<'a>(ptr: *const ffi::dnv_vista_sdk_locations_t) -> &'a Self {
        // SAFETY: caller guarantees ptr is non-null and valid. `Locations` is `#[repr(transparent)]`.
    ...
    pub fn relative_location_at(&self, index: usize) -> Option<&RelativeLocation> {
        ...
        Some(unsafe { RelativeLocation::from_ptr(ptr) })
    }
    ...
    pub fn from_string(&self, location_str: &str) -> Option<Location> {
        ...
        std::ptr::NonNull::new(ptr).map(Location)
    }
    ...
    pub fn from_string_with_errors(&self, location_str: &str) -> (Option<Location>, ParsingErrors) {
        ...
        let location = std::ptr::NonNull::new(ptr).map(Location);
        (
            location,
...
// SAFETY: Locations is a borrowed view (`repr(transparent)`). Sync only, not Send.
