...
impl LocalIdRef {
    ...
    pub(crate) unsafe fn from_ptr<'a>(ptr: *const ffi::dnv_vista_sdk_local_id_t) -> &'a Self {
        // SAFETY: caller guarantees ptr is non-null and valid. `LocalIdRef` is `#[repr(transparent)]`.
    ...
    pub fn primary_item(&self) -> &GmodPathRef {
        ...
        unsafe { GmodPathRef::from_ptr(ptr) }
    }
    ...
    pub fn metadata_tag(&self, name: CodebookName) -> Option<&MetadataTagRef> {
        ...
        if ptr.is_null() {
            ...
            // SAFETY: ptr is non-null (checked above) and valid for `self`'s lifetime.
            Some(unsafe { MetadataTagRef::from_ptr(ptr) })
        }
    }
    ...
    pub fn builder(&self) -> &LocalIdBuilderRef {
        ...
        // SAFETY: ptr is non-null (checked above) and valid for `self`'s lifetime.
        unsafe { LocalIdBuilderRef::from_ptr(ptr) }
    }
}
...
impl std::ops::Deref for LocalId {
    ...
    fn deref(&self) -> &LocalIdRef {
        ...
        unsafe { LocalIdRef::from_ptr(self.0.as_ptr()) }
    }
}
...
impl LocalId {
    ...
    pub fn from_string_with_errors(local_id_str: &str) -> (Option<Self>, ParsingErrors) {
        let c_str = match CString::new(local_id_str) {
            Ok(c) => c,
            Err(_) => return (None, ParsingErrors::empty()),
        };
...
// SAFETY: LocalIdRef is a borrowed view (`repr(transparent)`). Sync only, not Send.
unsafe impl Sync for LocalIdRef {}

// SAFETY: LocalId owns its heap-allocated C++ object exclusively.
unsafe impl Send for LocalId {}
unsafe impl Sync for LocalId {}
