...
impl Gmod {
    ...
    pub(crate) unsafe fn from_ptr<'a>(ptr: *const ffi::dnv_vista_sdk_gmod_t) -> &'a Self {
        // SAFETY: caller guarantees ptr is non-null and valid. `Gmod` is `#[repr(transparent)]`.
    ...
    pub fn root_node(&self) -> &GmodNodeRef {
        // SAFETY: self.as_ffi_ptr() is non-null and valid for the lifetime of `self`.
        let ptr = unsafe { ffi::dnv_vista_sdk_gmod_root_node(self.as_ffi_ptr()) };
        // SAFETY: ptr is non-null (root node always exists) and valid for `self`'s lifetime.
        unsafe { GmodNodeRef::from_ptr(ptr) }
    }

    /// Returns the node identified by `code`, or `Err` if not found.
    pub fn get_node(&self, code: &str) -> Result<&GmodNodeRef, VistaError> {
        ...
        if ptr.is_null() {
            ...
            Ok(unsafe { GmodNodeRef::from_ptr(ptr) })
        }
    }
    ...
    pub fn node_at(&self, index: usize) -> Option<&GmodNodeRef> {
        // SAFETY: self.as_ffi_ptr() is non-null and valid for the lifetime of `self`.
    ...
    where
        F: FnMut(&[&GmodNodeRef], &GmodNodeRef) -> TraversalHandlerResult,
    {
            ...
            F: FnMut(&[&GmodNodeRef], &GmodNodeRef) -> TraversalHandlerResult,
        {
            ...
            let parent_nodes: Vec<&GmodNodeRef> = if parent_count == 0 {
                Vec::new()
            } else {
                std::slice::from_raw_parts(parents, parent_count)
                    .iter()
                    .map(|p| GmodNodeRef::from_ptr(*p))
                    .collect()
            };
            let current = GmodNodeRef::from_ptr(node);
...
// SAFETY: Gmod is a borrowed view (`repr(transparent)`). Sync only, not Send.
unsafe impl Sync for Gmod {}
