...
impl Codebooks {
    ...
    pub(crate) unsafe fn from_ptr<'a>(ptr: *const ffi::dnv_vista_sdk_codebooks_t) -> &'a Self {
        // SAFETY: caller guarantees ptr is non-null and valid. `Codebooks` is `#[repr(transparent)]`.
...
impl Index<CodebookName> for Codebooks {
    ...
    fn index(&self, name: CodebookName) -> &Codebook {
        ...
        // SAFETY: ptr is non-null (the C API returns non-null for valid codebook names) and valid for `self`'s lifetime.
        unsafe { Codebook::from_ptr(ptr) }
    }
}

// SAFETY: Codebooks is a borrowed view (`repr(transparent)`). Sync only, not Send.
unsafe impl Sync for Codebooks {}
