...
pub fn last_error() -> VistaError {
    // SAFETY: thread-local getter, no arguments, always safe to call.
    ...
    let message = if ptr.is_null() {
        ...
        // SAFETY: ptr is non-null and owned by the library for the thread's lifetime.
...
pub fn clear_error() {
    // SAFETY: thread-local setter, no arguments, always safe to call.
