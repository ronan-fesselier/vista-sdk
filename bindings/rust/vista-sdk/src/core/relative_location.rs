...
impl RelativeLocation {
    ...
    pub(crate) unsafe fn from_ptr<'a>(
        ptr: *const ffi::dnv_vista_sdk_relative_location_t,
    ) -> &'a Self {
        // SAFETY: caller guarantees ptr is non-null and valid. `RelativeLocation` is `#[repr(transparent)]`.
    ...
    pub fn location_value(&self) -> &str {
        ...
        // SAFETY: ptr is non-null and borrowed from `self` for its lifetime.
        let value_ptr = unsafe { ffi_location::dnv_vista_sdk_location_value(ptr) };
        assert!(
            !value_ptr.is_null(),
            "dnv_vista_sdk_location_value returned NULL"
        );
        // SAFETY: value_ptr is non-null and borrowed from `self` for its lifetime.
...
// SAFETY: RelativeLocation is a borrowed view (`repr(transparent)`). Sync only, not Send.
unsafe impl Sync for RelativeLocation {}
