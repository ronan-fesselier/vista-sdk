...
///
/// Borrowed from the parent [`crate::core::gmod_node::GmodNodeRef`], valid as long as it is.
#[repr(transparent)]
...
impl GmodNodeMetadata {
    ...
    pub(crate) unsafe fn from_ptr<'a>(
        ptr: *const ffi::dnv_vista_sdk_gmod_node_metadata_t,
    ) -> &'a Self {
        // SAFETY: caller guarantees ptr is non-null and valid. `GmodNodeMetadata` is `#[repr(transparent)]`.
    ...
    pub fn install_substructure(&self) -> Option<bool> {
        ...
        // SAFETY: self is non-null, and `out` is a valid writable pointer to a local.
        let ok = unsafe {
    ...
    pub fn normal_assignment_name_at(&self, index: usize) -> Option<(&str, &str)> {
        ...
        // SAFETY: key_ptr is non-null (checked above) and owned by `self` for its lifetime.
...
// SAFETY: GmodNodeMetadata is a borrowed view (`repr(transparent)`). Sync only, not Send.
unsafe impl Sync for GmodNodeMetadata {}
