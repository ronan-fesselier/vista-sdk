...
fn bench(c: &mut Criterion) {
    ...
    let local_id = LocalId::from_string(&local_id_str).expect("LocalId string must parse");

    c.bench_function("ByShortId", |b| {
        b.iter(|| black_box(list.from_short_id(&short_id)))
    });

    c.bench_function("ByLocalId", |b| {
        b.iter(|| black_box(list.from_local_id(&local_id)))
    });
}
