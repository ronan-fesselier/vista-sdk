...
fn bench(c: &mut Criterion) {
    ...
    c.bench_function("StdUnorderedMap", |b| {
        b.iter(|| black_box(keys.iter().all(|k| hash_map.contains_key(k))))
    });

    c.bench_function("StdMap", |b| {
        b.iter(|| black_box(keys.iter().all(|k| btree_map.contains_key(k))))
    });

    c.bench_function("Codebooks", |b| {
        b.iter(|| {
            black_box(&codebooks[CodebookName::Quantity]);
            black_box(&codebooks[CodebookName::Position]);
            black_box(&codebooks[CodebookName::State]);
            black_box(&codebooks[CodebookName::Detail]);
        })
    });
}
