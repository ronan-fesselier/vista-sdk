...
[![Rust Workflow Status](https://img.shields.io/github/actions/workflow/status/dnv-opensource/vista-sdk/build-rust-bindings.yml?branch=main&label=Rust+Build+%26+Test)](https://github.com/dnv-opensource/vista-sdk/actions)
...
fn main() {
    let fuel_consumed = Decimal::from_str("12.450").expect("valid");   // tonnes
    let price_per_tonne = Decimal::from_str("615.30").expect("valid"); // USD
...
fn main() {
    let dto = DateTimeOffset::from_str("2026-09-13T19:11:23+02:00").expect("valid");
...
fn main() {
    ...
    let local_id = OwnedLocalIdBuilder::create(version)
...
| `core_gmodsubset`           | Build asset model subsets from Gmod paths               |
...
| `transport_datachannellist` | Parse and serialize DataChannelList packages            |
| `transport_timeseriesdata`  | Build and serialize TimeSeriesData packages             |
| `transport_sensorsdataflow` | End-to-end sensor data flow to ISO 19848                |
...
`OwnedGmodPathQueryBuilder`, `OwnedMetadataTagsQueryBuilder`, and `OwnedLocalIdQueryBuilder` provide a fluent, immutable builder API for matching `GmodPath` / `LocalId` instances against structural and metadata criteria. Useful for filtering incoming sensor data or Local IDs against a set of rules without re-parsing them.
...
fn main() {
    ...
    let base_path  = OwnedGmodPath::from_short_path("411.1/C101", gmod, locations).expect("valid");
    ...
    let a = OwnedLocalId::from_string("/dnv-v2/vis-3-11a/411.1/C101.31-2/meta/qty-temperature")
        .expect("valid");
    let b = OwnedLocalId::from_string("/dnv-v2/vis-3-11a/411.1/C101.31-2/meta/qty-pressure")
        .expect("valid");
...
`OwnedLocalId::from_string_with_errors` returns both the parsed value (if valid) and any accumulated errors, mirroring the C++ `LocalId::fromString` with error output parameter.
...
fn main() {
    ...
    for s in test_strings {
        let (local_id, errors) = OwnedLocalId::from_string_with_errors(s);
        println!("Parsing: \"{}\"", s);
...
First 10 nodes:
  ...
  1044     - Excessive roll predicting
  C101.311 - Cylinder head
  C102.313 - Intermediate pressure turbine labyrith sealing
  973.51   - Seating - seats
  C1057.12 - Jacking by rack and pinion arrangements
  C327.2   - Housing arrangement
  1263.44  - Drilling fluid degassing
```
...
fn main() {
    ...
    let path     = OwnedGmodPath::from_short_path("411.1/C101.31-2", gmod, locations).expect("valid");
    let qty_tag  = codebooks[CodebookName::Quantity]
        .create_tag("temperature")
        .expect("valid");
    ...
    property.set_range(&Range::new(0.0, 600.0));
    property.set_unit(&Unit::new("°C"));
    let channel      = DataChannel::new(&channel_id, &property);

    let imo        = ImoNumber::from_str("1234567").expect("valid");
    let ship_id    = ShipId::from_imo_number(imo);
    let config_ref = ConfigurationReference::new(
        "dcl-001",
        DateTimeOffset::from_str("2026-09-13T19:11:23Z").expect("valid"),
    );
...
fn main() {
    ...
    let ts      = DateTimeOffset::from_str("2026-09-13T10:00:00Z").expect("valid");
    let ds      = TabularDataSet::new(ts, &["87.3"], None);
    let tabular = TabularData::new(&[&ch], &[&ds]);

    let mut tsd = TimeSeriesData::new();
    tsd.set_tabular_data(&[&tabular]);
...
The crate uses three distinct conventions depending on the nature of the failure:

| Pattern                 | When                                                                                               | Example                                                                 |
| ----------------------- | -------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------- |
| `Result<T, VistaError>` | Operation failed in the C layer: error kind and message from `last_error()`                        | `OwnedLocalIdBuilder::build(...)`                                       |
| `Option<T>`             | Expected failure: input may legitimately be unparseable, mirroring `std::optional` on the C++ side | `OwnedLocalId::from_string(...)`, `OwnedGmodPath::from_short_path(...)` |
