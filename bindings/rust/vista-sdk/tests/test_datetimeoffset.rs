...
#[test]
fn date_time_offset_to_system_time_round_trip() {
    let dto = "2026-07-14T09:00:00Z".parse::<DateTimeOffset>().unwrap();
    let st = dto.to_system_time();
    let recovered = DateTimeOffset::from_system_time(st);
    assert_eq!(dto.to_epoch_seconds(), recovered.to_epoch_seconds());
    assert_eq!(recovered.total_offset_minutes(), 0);
}

#[test]
fn date_time_offset_to_system_time_discards_offset() {
    let tokyo = "2026-07-14T18:00:00+09:00"
        .parse::<DateTimeOffset>()
        .unwrap();
    let utc = "2026-07-14T09:00:00Z".parse::<DateTimeOffset>().unwrap();
    assert_eq!(tokyo.to_system_time(), utc.to_system_time());
}

#[test]
fn date_time_offset_from_system_time_epoch() {
    let dto = DateTimeOffset::from_system_time(std::time::UNIX_EPOCH);
    assert_eq!(dto.to_epoch_seconds(), 0);
    assert_eq!(dto.total_offset_minutes(), 0);
}

#[test]
fn date_time_offset_to_system_time_preserves_sub_millisecond_ticks() {
    // 100 ticks == 10 microseconds, well below millisecond precision.
    let dto = DateTimeOffset::epoch() + TimeSpan::from_ticks(100);
    let st = dto.to_system_time();
...
#[test]
fn date_time_offset_to_system_time_before_epoch_with_fraction() {
    // 1969-12-31T23:59:59.500Z, i.e. -500ms
    let dto = DateTimeOffset::from_epoch_millis(-500);
    let st = dto.to_system_time();
