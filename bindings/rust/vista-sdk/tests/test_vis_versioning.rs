...
fn convert_path_across_versions() {
    ...
    for (input_path, expected_path) in cases {
        let source_path = GmodPath::from_short_path(input_path, source_gmod, source_locations)
...
fn convert_path_out_of_scope_returns_err() {
    ...
    let source_path =
        GmodPath::from_short_path("244.1i/H101.111/H401", gmod, locations).expect("should parse");
...
fn convert_local_id_across_versions() {
    ...
    for (source_str, target_str) in cases {
        let source_local_id =
            LocalId::from_string(source_str).expect("should parse source LocalId");
        let target_local_id =
            LocalId::from_string(target_str).expect("should parse target LocalId");
...
fn convert_local_id_builder_across_versions() {
    ...
    let source_local_id = LocalId::from_string(
        "/dnv-v2/vis-3-4a/411.1/C101/sec/411.1/C101.64i/S201/meta/cnt-condensate",
    )
