...
use vista_sdk::core::gmod_path::{GmodPath, GmodPathRef};
...
impl AssetModel {
    ...
    fn add_path(&mut self, path: &GmodPathRef, gmod: &Gmod, locations: &Locations) {
        ...
        for (i, node) in all_nodes.iter().enumerate() {
            ...
            let (sub_path_string, display_name) = if depth == 0 {
                ...
                let sub_path = GmodPath::from_full_path(&node_id, gmod, locations);
    ...
    fn from_path_strings(vis_version: VisVersion, path_strings: &[&str]) -> Self {
        ...
        for &path_str in path_strings {
            match GmodPath::from_short_path(path_str, gmod, locations) {
...
fn main() {
...

    {
        ...
        let mut paths: Vec<GmodPath> = Vec::new();
        let gmod = vis.gmod(vis_version).expect("valid vis version");
        let locations = vis.locations(vis_version).expect("valid vis version");

        for &lid_str in local_id_strings {
            if let Some(local_id) = LocalId::from_string(lid_str) {
                let full = local_id.primary_item().to_full_path_string();
                if let Some(p) = GmodPath::from_full_path(&full, gmod, locations) {
                    paths.push(p);
                }
                if let Some(secondary) = local_id.secondary_item() {
                    let full2 = secondary.to_full_path_string();
                    if let Some(p) = GmodPath::from_full_path(&full2, gmod, locations) {
                        paths.push(p);
                    }
                }
            }
