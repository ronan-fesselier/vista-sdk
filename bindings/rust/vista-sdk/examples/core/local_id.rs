...
fn main() {
...

    {
        ...
        let local_id = LocalId::from_string(local_id_str);
...

    {
        ...
        for str in test_strings {
            let (local_id, errors) = LocalId::from_string_with_errors(str);
...

    {
        ...
        if let (Some(primary_item), Some(qty_tag)) = (primary_item, qty_tag) {
            let normal_local_id = LocalIdBuilder::create(vis.latest())
                .with_primary_item(&primary_item)
                .with_metadata_tag(&qty_tag)
                .build()
                .expect("builder with primary item and metadata tag should be valid");
            ...
            let verbose_local_id = LocalIdBuilder::create(vis.latest())
                .with_primary_item(&primary_item)
                .with_metadata_tag(&qty_tag)
...

    {
        ...
        if let (Some(primary_item), Some(qty_tag)) = (primary_item, qty_tag) {
            let valid_builder = LocalIdBuilder::create(vis.latest())
                .with_primary_item(&primary_item)
                .with_metadata_tag(&qty_tag);
            ...
            // Empty builder
            let empty_builder = LocalIdBuilder::create(vis.latest());
            println!("\nEmpty builder:");
            ...
            let incomplete_builder =
                LocalIdBuilder::create(vis.latest()).with_primary_item(&primary_item);
...

    {
        ...
        for (label, local_id_str) in simple {
            let Some(local_id) = LocalId::from_string(local_id_str) else {
                continue;
            };
        ...
        {
            let primary_item = GmodPath::from_short_path("411.1/C101.31-2", gmod, locations);
            let secondary_item = GmodPath::from_short_path("411.1/C101.31-5", gmod, locations);
            let qty_tag = codebooks[CodebookName::Quantity].create_tag("temperature");
            ...
            if let (
                ...
                let builder = LocalIdBuilder::create(vis.latest())
                    .with_primary_item(&primary_item)
                    .with_secondary_item(&secondary_item)
                    .with_metadata_tag(&qty_tag)
                    .with_metadata_tag(&cnt_tag)
        ...
        {
            let primary_item = GmodPath::from_short_path("411.1/C101.63/S206", gmod, locations);
            let qty_tag = codebooks[CodebookName::Quantity].create_tag("temperature");
