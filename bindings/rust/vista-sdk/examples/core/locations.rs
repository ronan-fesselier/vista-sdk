...
fn main() {
...

    {
        ...
        for loc_str in &valid_locations {
            ...
            match locations.from_string(loc_str) {
...

    {
        ...
        for loc_str in &invalid_locations {
            let (location, errors) = locations.from_string_with_errors(loc_str);
...

    {
        ...
        let user_input = "3SU";
        let parsed_location = locations.from_string(user_input);
        ...
        match parsed_location {
            Some(parsed) => {
                ...
                if let Some(revalidated) = locations.from_string(modified.value()) {
