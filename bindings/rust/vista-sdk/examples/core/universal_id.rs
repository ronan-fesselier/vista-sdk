...
fn main() {
...

    {
        ...
        let primary_item = GmodPath::from_short_path("411.1/C101.31-2", gmod, locations);
        ...
        if let (Some(primary_item), Some(qty_tag)) = (primary_item, qty_tag) {
            let local_id_builder = LocalIdBuilder::create(vis.latest())
                .with_primary_item(&primary_item)
                .with_metadata_tag(&qty_tag);

            let universal_id = UniversalIdBuilder::create(vis.latest())
...

    {
        ...
        let primary_item = GmodPath::from_short_path("621.21/S90", gmod, locations);
        let secondary_item = GmodPath::from_short_path("411.1/C101", gmod, locations);
        ...
        ) = (primary_item, secondary_item, qty_tag, cnt_tag, pos_tag)
        {
            ...
            println!("  Naming entity: {}", UniversalIdRef::naming_entity());
            println!("  IMO Number   : {}", universal_id.imo_number());
        }
...

    {
        ...
        let universal_id = UniversalIdBuilder::from_string(universal_id_str);
...

    {
        ...
        for str in test_strings {
            let (universal_id, errors) = UniversalIdBuilder::from_string_with_errors(str);
...

    {
        ...
        let original = UniversalIdBuilder::from_string(
            "data.dnv.com/IMO1234567/dnv-v2/vis-3-4a/411.1/C101.31-2/meta/qty-temperature",
        );
...

    {
        ...
        let universal_id = UniversalIdBuilder::from_string(
            "data.dnv.com/IMO9074729/dnv-v2/vis-3-4a/621.21/S90/sec/411.1/C101/meta/qty-mass/cnt-fuel.oil/pos-inlet",
        );
...

    {
        ...
        let uid1 = UniversalIdBuilder::from_string(
            "data.dnv.com/IMO1234567/dnv-v2/vis-3-4a/411.1/C101.31-2/meta/qty-temperature",
        );
        let uid2 = UniversalIdBuilder::from_string(
            "data.dnv.com/IMO1234567/dnv-v2/vis-3-4a/411.1/C101.31-2/meta/qty-temperature",
        );
        let uid3 = UniversalIdBuilder::from_string(
            "data.dnv.com/IMO9074729/dnv-v2/vis-3-4a/411.1/C101.31-2/meta/qty-temperature",
        );
        let uid4 = UniversalIdBuilder::from_string(
...

    {
        ...
        if let (Some(primary_item), Some(qty_tag)) = (primary_item, qty_tag) {
            ...
            let valid_builder = UniversalIdBuilder::create(vis.latest())
                .with_imo_number(&imo_number)
                .with_local_id(&local_id_builder);
            ...
            let missing_imo =
                UniversalIdBuilder::create(vis.latest()).with_local_id(&local_id_builder);
            println!("\nBuilder missing IMO number:");
            println!("  Is valid: {}", missing_imo.is_valid());

            let missing_local_id =
                UniversalIdBuilder::create(vis.latest()).with_imo_number(&imo_number);
...

    {
        ...
        for (vessel_name, imo_number, sensor_path, quantity) in sensors {
            ...
            let path = GmodPath::from_short_path(sensor_path, gmod, locations);
...

    {
        ...
        if let Some(original) = original {
            ...
            let new_imo = ImoNumber::create(9074729).expect("9074729 should be a valid IMO number");
            let path = GmodPath::from_short_path("621.21/S90", gmod, locations);
            let tag = codebooks[CodebookName::Quantity].create_tag("mass");

            if let (Some(path), Some(tag)) = (path, tag) {
                let new_local_id_builder = LocalIdBuilder::create(vis.latest())
                    .with_primary_item(&path)
                    .with_metadata_tag(&tag);
