...
fn create_data_channel_list() -> DataChannelListPackage {
    ...
    let local_id1 = LocalIdBuilder::from_string(
    ...
    let restriction1 = Restriction::new()
        ...
        .with_min_inclusive(-50.0);
    let format1 = Format::new("Decimal").with_restriction(&restriction1);
    let dct1 = DataChannelType::new("Inst").with_update_cycle(1.0);

    let range1 = Range::new(0.0, 150.0);
    let unit1 = Unit::new("°C").with_quantity_name("Temperature");

    let prop1 = Property::new(&dct1, &format1)
        .with_range(&range1)
        .with_unit(&unit1)
        .with_quality_coding("OPC_QUALITY")
        .with_name("Main Engine Air Cooler Temperature");
    ...
    let unit2 = Unit::new("kW").with_quantity_name("Power");

    let prop2 = Property::new(&dct2, &format2)
        .with_range(&range2)
        .with_unit(&unit2)
        .with_name("Generator Power Output");
    ...
    let prop3 = Property::new(&dct3, &format3).with_name("Diesel Oil Stop Command");
    ...
    let config_ref = ConfigurationReference::new(
        "DataChannelList-v1",
        "2026-08-28T00:00:00Z".parse::<DateTimeOffset>().unwrap(),
    );
...
fn main() {
...

    {
        ...
        let base_time = "2026-08-28T10:00:00Z".parse::<DateTimeOffset>().unwrap();
        let base_plus_5 = "2026-08-28T10:05:00Z".parse::<DateTimeOffset>().unwrap();
        ...
        let ts_header = TsdHeader::new(&ship_id)
            .with_time_span(&time_span)
            .with_author("Vista SDK Sample");

        let ts_data = TimeSeriesData::new()
            .with_data_configuration(&configuration)
            .with_tabular_data(&tabular_data_list.iter().collect::<Vec<_>>());
        ...
        let validation = ts_data.validate(
            &dcl_package,
            &|_ts, _dc, _val, _qual| ValidateResult::Ok,
            &|_ts, _dc, _val, _qual| ValidateResult::Ok,
        );
        ...
        let faulty_ts_data = TimeSeriesData::new()
            .with_data_configuration(&configuration)
            .with_tabular_data(&[&faulty_tabular]);

        println!("  Reporting TEMP001 = 999.9 degC (Restriction.MaxInclusive is 200)");

        let faulty_validation = faulty_ts_data.validate(
            &dcl_package,
            &|_ts, _dc, _val, _qual| ValidateResult::Ok,
            &|_ts, _dc, _val, _qual| ValidateResult::Ok,
        );
