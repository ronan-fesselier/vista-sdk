...
extern "C"
{
    ...
    /**
     * @brief Compare two UniversalId handles for equality
     * @param a First handle
     * @param b Second handle
     * @return 1 if both represent the same UniversalId, 0 otherwise or if either is NULL
     */
    DNV_VISTA_SDK_C_API int dnv_vista_sdk_universal_id_equals(
        const dnv_vista_sdk_universal_id_t* a, const dnv_vista_sdk_universal_id_t* b);

    /**
     * @brief Convert to string representation
     * @return Owned, null-terminated string, must be released with
     *         dnv_vista_sdk_string_free, or NULL if `universalId` is NULL
     */
