...
extern "C"
{
     ...
     *         dnv_vista_sdk_string_free, or NULL if `node` is NULL
     */
    DNV_VISTA_SDK_C_API char* dnv_vista_sdk_gmod_node_to_string(const dnv_vista_sdk_gmod_node_t* node);

    /**
     * @brief Compare two GmodNode handles for equality
     * @param a First handle
     * @param b Second handle
     * @return 1 if both represent the same node, 0 otherwise or if either is NULL
     */
    DNV_VISTA_SDK_C_API int dnv_vista_sdk_gmod_node_equals(
        const dnv_vista_sdk_gmod_node_t* a, const dnv_vista_sdk_gmod_node_t* b);
