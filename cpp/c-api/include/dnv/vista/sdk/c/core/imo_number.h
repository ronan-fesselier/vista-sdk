...
extern "C"
{
    ...
    /**
     * @brief Construct an IMO number from a string, with checksum validation
     * @param value String with or without "IMO" prefix (e.g. "IMO9074729" or "9074729")
     * @return Owned handle, must be released with dnv_vista_sdk_imo_number_free, or
     *         NULL if `value` is invalid (bad format or failed checksum) or NULL
     * @details Mirrors the throwing `ImoNumber(std::string_view)` constructor - sets the
     *          last-error message/kind on failure, unlike dnv_vista_sdk_imo_number_from_string
     *          which fails silently (mirrors the non-throwing `ImoNumber::fromString`)
     */
    DNV_VISTA_SDK_C_API dnv_vista_sdk_imo_number_t* dnv_vista_sdk_imo_number_create_from_string(const char* value);
    ...
    /**
     * @brief Get the raw integer value of an IMO number
     * @param imoNumber Handle obtained from this API, must not be NULL
     * @return Seven-digit integer value (e.g. 9074729)
     */
    DNV_VISTA_SDK_C_API int dnv_vista_sdk_imo_number_value(const dnv_vista_sdk_imo_number_t* imoNumber);
