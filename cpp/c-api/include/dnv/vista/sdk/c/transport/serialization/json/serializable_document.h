...
extern "C"
{
    ...
    /**
     * @brief Get a mutable array element by index
     * @details Same as dnv_vista_sdk_serializable_document_array_at except the returned handle is
     *          non-const and may be passed to a mutating call (set / push_back) on the element.
     *          Only the outer container is required to be non-const.
     * @param doc Handle obtained from this API, must be an array
     * @param index Zero-based index, must be < dnv_vista_sdk_serializable_document_array_size(doc)
     * @return Borrowed mutable pointer, valid as long as `doc` is valid, or NULL if `doc` is
     *         NULL, not an array, or `index` is out of range
     */
    DNV_VISTA_SDK_C_API dnv_vista_sdk_serializable_document_t* dnv_vista_sdk_serializable_document_array_at_mut(
        dnv_vista_sdk_serializable_document_t* doc, size_t index);
    ...
    /**
     * @brief Get a mutable object entry's value by insertion-order index
     * @details Same as dnv_vista_sdk_serializable_document_object_value_at except the returned
     *          handle is non-const and may be passed to a mutating call on the value.
     * @return Borrowed mutable pointer, valid as long as `doc` is valid, or NULL if `doc` is
     *         NULL, not an object, or `index` is out of range
     */
    DNV_VISTA_SDK_C_API dnv_vista_sdk_serializable_document_t* dnv_vista_sdk_serializable_document_object_value_at_mut(
        dnv_vista_sdk_serializable_document_t* doc, size_t index);
    ...
    /**
     * @brief Look up a key in this object, returning a mutable handle
     * @details Same as dnv_vista_sdk_serializable_document_find except the returned handle is
     *          non-const and may be passed to a mutating call (set / push_back) on the value.
     * @return Borrowed mutable pointer, valid as long as `doc` is valid, or NULL if `doc` is
     *         NULL, not an object, or `key` is not present
     */
    DNV_VISTA_SDK_C_API dnv_vista_sdk_serializable_document_t* dnv_vista_sdk_serializable_document_find_mut(
        dnv_vista_sdk_serializable_document_t* doc, const char* key);
