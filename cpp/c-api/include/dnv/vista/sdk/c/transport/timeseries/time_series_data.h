...
extern "C"
{
     ...
     *                         string is read by the caller after the callback returns - it must
     *                         remain valid until the caller has copied it. It is never freed by
     *                         the caller. Ownership stays with the callback. A string literal or
     *                         a buffer that outlives the callback invocation are valid choices.
     *                         May be left untouched (defaults to a generic message)
