...
dnv_vista_sdk_imo_number_t* dnv_vista_sdk_imo_number_create_from_string(const char* value)
{
    if (value == nullptr)
    {
        c::setLastError("value must not be null", DNV_VISTA_SDK_ERROR_INVALID_ARGUMENT);
...
int dnv_vista_sdk_imo_number_value(const dnv_vista_sdk_imo_number_t* imoNumber)
{
    return toImoNumber(imoNumber)->value();
}
