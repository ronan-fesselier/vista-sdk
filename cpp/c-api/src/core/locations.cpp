...
dnv_vista_sdk_location_t* dnv_vista_sdk_locations_from_string(
    const dnv_vista_sdk_locations_t* locations, const char* locationStr)
{
    if (locations == nullptr || locationStr == nullptr)
    {
        c::setLastError("locations and locationStr must not be null", DNV_VISTA_SDK_ERROR_INVALID_ARGUMENT);
        return nullptr;
    }
    ...
    {
        c::setLastError("invalid location string", DNV_VISTA_SDK_ERROR_INVALID_ARGUMENT);
    }

    return fromLocation(std::move(location));
}

dnv_vista_sdk_location_t* dnv_vista_sdk_locations_from_string_with_errors(
    const dnv_vista_sdk_locations_t* locations, const char* locationStr, dnv_vista_sdk_parsing_errors_t** outErrors)
{
    ...
    {
        c::setLastError("locations, locationStr and outErrors must not be null", DNV_VISTA_SDK_ERROR_INVALID_ARGUMENT);
        return nullptr;
    }
