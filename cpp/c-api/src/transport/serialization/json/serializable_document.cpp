...
using dnv::vista::sdk::c::fromSerializableDocumentRefMutable;
using dnv::vista::sdk::c::toSerializableDocument;
using dnv::vista::sdk::c::toSerializableDocumentMutable;
...
dnv_vista_sdk_serializable_document_t* dnv_vista_sdk_serializable_document_array_at_mut(
    dnv_vista_sdk_serializable_document_t* doc, size_t index)
{
    ...
    auto& arr = toSerializableDocumentMutable(doc)->asArray();
    if (index >= arr.size())
    {
        c::setLastError("index out of range", DNV_VISTA_SDK_ERROR_OUT_OF_RANGE);
        return nullptr;
    ...
    return fromSerializableDocumentRefMutable(arr[index]);
}
...
dnv_vista_sdk_serializable_document_t* dnv_vista_sdk_serializable_document_object_value_at_mut(
    dnv_vista_sdk_serializable_document_t* doc, size_t index)
{
    if (doc == nullptr || !toSerializableDocument(doc)->isObject())
    {
    ...
    auto& obj = toSerializableDocumentMutable(doc)->asObject();
    if (index >= obj.size())
    {
        c::setLastError("index out of range", DNV_VISTA_SDK_ERROR_OUT_OF_RANGE);
        return nullptr;
    ...
    return fromSerializableDocumentRefMutable(obj[index].second);
}
...
dnv_vista_sdk_serializable_document_t* dnv_vista_sdk_serializable_document_find_mut(
    dnv_vista_sdk_serializable_document_t* doc, const char* key)
{
    ...
    auto* found = toSerializableDocumentMutable(doc)->find(key);
    if (found == nullptr)
    {
        return nullptr;
    }

    return fromSerializableDocumentRefMutable(*found);
}
