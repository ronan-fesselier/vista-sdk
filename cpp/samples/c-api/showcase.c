 ...
 *          ImoNumber, Gmod node lookup, Gmod depth-first traversal, GmodPath, LocalId,
 *          LocalId MQTT, UniversalId, ISO 19848 value validation, GmodPath/LocalId version
 *          conversion, GmodPathQuery, LocalIdQuery, DataChannelList, TimeSeriesData
 *          with business-rule callbacks, and DTO-level patching before serialization
 */
...
static vista_traversal_result_t onTraverseCount(
    const vista_gmod_node_t* const* parents, size_t parent_count, const vista_gmod_node_t* node, void* userdata)
{
    (void)parents;
    (void)parent_count;
    (void)node;
    (*(size_t*)userdata)++;
    return VISTA_TRAVERSAL_CONTINUE;
}
...
static vista_traversal_result_t onTraverseFirst(
    const vista_gmod_node_t* const* parents, size_t parent_count, const vista_gmod_node_t* node, void* userdata)
{
    (void)parents;
    (void)parent_count;
    TraverseFirstState* s = (TraverseFirstState*)userdata;
    if (s->count < 10)
    {
        s->codes[s->count++] = vista_gmod_node_code(node);
    }
    return s->count >= 10 ? VISTA_TRAVERSAL_STOP : VISTA_TRAVERSAL_CONTINUE;
}
...
static int onTabularDataPoint(
    ...
    const char* quality,
    vista_tsd_validation_errors_t* outErrors,
    void* userdata)
{
    (void)timeStamp;
    (void)userdata;
    ...
    if (range != NULL)
    {
        ...
        if (vista_decimal_from_string(valueStr, &decVal))
        {
            ...
            if (vista_decimal_compare(decVal, threshold) > 0)
            {
                ...
                vista_tsd_validation_errors_add(outErrors, "value approaching upper limit");
...
int main(void)
{
    ...
    {
        printf("6. Gmod: Depth-first traversal\n");
        printf("-------------------------------------------------------\n");

        /* Count all nodes visited depth-first */
        size_t traverseCount = 0;
        vista_gmod_traverse(gmod, onTraverseCount, 1, &traverseCount);
        printf("Total nodes visited (DFS): %zu\n", traverseCount);

        /* First 10 nodes in DFS order */
        TraverseFirstState firstState;
        firstState.count = 0;
        vista_gmod_traverse(gmod, onTraverseFirst, 1, &firstState);
        printf("First 10 nodes in DFS order:\n");
        for (size_t i = 0; i < firstState.count; ++i)
        {
            printf("  %s\n", firstState.codes[i]);
        }
    ...
    {
        printf("7. GmodPath: Parsing a short path\n");
        printf("-------------------------------------------------------\n");
    ...
    {
        printf("8. LocalId: Parsing and inspecting a LocalId string\n");
        printf("-------------------------------------------------------\n");
    ...
    {
        printf("9. LocalId MQTT: MQTT-compatible topic formatting\n");
        printf("-------------------------------------------------------\n");
    ...
    {
        printf("10. UniversalId: Globally unique sensor identifier\n");
        printf("-------------------------------------------------------\n");
    ...
    {
        printf("11. ISO19848: Validating a value against a FormatDataType\n");
        printf("-------------------------------------------------------\n");
    ...
    {
        printf("12. VIS versioning: Converting a GmodPath between versions\n");
        printf("-------------------------------------------------------\n");
    ...
    {
        printf("13. VIS versioning: Converting a LocalId between versions\n");
        printf("-------------------------------------------------------\n");
    ...
    {
        printf("14. GmodPathQuery: Matching a base path while ignoring locations\n");
        printf("-------------------------------------------------------\n");
    ...
    {
        printf("15. LocalIdQuery: Combining a primary-item path query with a tags query\n");
        printf("-------------------------------------------------------\n");
    ...
    {
        printf("16. DataChannelList: Building a package and serializing to JSON\n");
        printf("-------------------------------------------------------\n");
    ...
    vista_tsd_data_package_t* tsdDataPackage = NULL;

    {
        printf("17. TimeSeriesData: Building a package and cross-validating against the DataChannelList\n");
        printf("-------------------------------------------------------\n");
        ...
        tsdDataPackage = vista_tsd_data_package_create(package);
        ...
        char* json = vista_tsd_data_package_to_json(tsdDataPackage, 1);
        printf("\nJSON output:\n%s\n", json);
        vista_string_free(json);
    ...
    if (dclListPackage != NULL)
    {
        printf("18. DTO-level manipulation: patching and serializing from the DTO\n");
        printf("------------------------------------------------------------------\n");

        /* The DTO is the serialization-facing representation of the package. Converting to it
           lets a caller patch values and serialize without rebuilding a validated domain
           package (which would require, for example, well-formed LocalIDs). */
        vista_dcl_dto_package_t* dto = vista_dcl_to_dto(dclListPackage);
        if (dto != NULL)
        {
            vista_dcl_dto_pkg_t* dtoPkg = vista_dcl_dto_package_get_pkg(dto);
            vista_dcl_dto_header_t* dtoHeader = vista_dcl_dto_pkg_get_header(dtoPkg);

            printf("Original author present: %s\n", vista_dcl_dto_header_has_author(dtoHeader) ? "yes" : "no");

            /* Patch scalar fields directly on the DTO. */
            vista_dcl_dto_header_set_author(dtoHeader, "export-pipeline");

            /* Patch the xs:any extension point (custom headers) through the
               SerializableDocument view. */
            vista_dcl_dto_header_ensure_custom_headers(dtoHeader);
            vista_serializable_document_t* customHeaders = vista_dcl_dto_header_get_custom_headers(dtoHeader);
            if (customHeaders != NULL)
            {
                vista_serializable_document_set(
                    customHeaders, "exportedBy", vista_serializable_document_from_string("vista-sdk-sample"));
                vista_serializable_document_set(
                    customHeaders, "exportTimestamp", vista_serializable_document_from_string("2026-07-16T00:00:00Z"));

                printf(
                    "Custom headers after patch (%zu entries):\n",
                    vista_serializable_document_object_size(customHeaders));
                for (size_t i = 0; i < vista_serializable_document_object_size(customHeaders); ++i)
                {
                    printf("  %s\n", vista_serializable_document_object_key_at(customHeaders, i));
                }
            }

            /* Serialize straight from the patched DTO. */
            char* dtoJson = vista_dcl_dto_to_json(dto, 1);
            printf("\nSerialized from the patched DTO:\n%s\n", dtoJson);
            vista_string_free(dtoJson);

            vista_dcl_dto_package_free(dto);
        }
        else
        {
            printf("to_dto failed: %s\n", vista_last_error_message());
        }
        printf("\n");
    }

    if (tsdDataPackage != NULL)
    {
        printf("19. TSD DTO-level manipulation: patching and serializing from the DTO\n");
        printf("-----------------------------------------------------------------------\n");

        /* Same pattern as section 18: convert to the DTO layer, patch directly, and
           serialize - no need to rebuild a validated domain package. */
        vista_tsd_dto_package_t* dto = vista_tsd_to_dto(tsdDataPackage);
        if (dto != NULL)
        {
            vista_tsd_dto_pkg_t* dtoPkg = vista_tsd_dto_package_get_pkg(dto);
            vista_tsd_dto_header_t* dtoHeader = vista_tsd_dto_pkg_get_header(dtoPkg);

            printf("Original author present: %s\n", vista_tsd_dto_header_has_author(dtoHeader) ? "yes" : "no");

            /* Patch author directly on the DTO header. */
            vista_tsd_dto_header_set_author(dtoHeader, "export-pipeline");
            printf("Author after patch     : %s\n", vista_tsd_dto_header_get_author(dtoHeader));

            /* Inspect the first TimeSeriesData entry. */
            size_t tsdCount = vista_tsd_dto_pkg_tsd_count(dtoPkg);
            printf("TimeSeriesData entries : %zu\n", tsdCount);

            if (tsdCount > 0)
            {
                vista_tsd_dto_tsd_t* tsd = vista_tsd_dto_pkg_tsd_at(dtoPkg, 0);

                /* Report tabular channel count. */
                size_t tabCount = vista_tsd_dto_tsd_tabular_count(tsd);
                if (tabCount > 0)
                {
                    vista_tsd_dto_tabular_t* tab = vista_tsd_dto_tsd_tabular_at(tsd, 0);
                    printf("  Tabular channel ids : %zu\n", vista_tsd_dto_tabular_channel_id_count(tab));
                    printf("  Tabular data sets   : %zu\n", vista_tsd_dto_tabular_data_set_count(tab));
                }

                /* Report event data set count if present. */
                if (vista_tsd_dto_tsd_has_event(tsd))
                {
                    vista_tsd_dto_event_t* event = vista_tsd_dto_tsd_get_event(tsd);
                    printf("  Event data sets     : %zu\n", vista_tsd_dto_event_data_set_count(event));
                }
            }

            /* Serialize straight from the patched DTO. */
            char* dtoJson = vista_tsd_dto_to_json(dto, 1);
            printf("\nSerialized from the patched TSD DTO:\n%s\n", dtoJson);
            vista_string_free(dtoJson);

            vista_tsd_dto_package_free(dto);
        }
        else
        {
            printf("to_dto failed: %s\n", vista_last_error_message());
        ...
        vista_tsd_data_package_free(tsdDataPackage);
    }
