...
namespace dnv::vista::sdk
{
    ...
    {
        if (m_name == CodebookName::Position && internal::string::isAllDigits(value))
        {
            return true;
        }
    ...
    inline std::optional<MetadataTag> Codebook::createTag(std::string_view value) const noexcept
    {
        ...
        {
            if (!internal::string::isUriUnreserved(value))
            {
                return std::nullopt;
            }
    ...
    {
        if (internal::string::isNullOrWhiteSpace(position) || !internal::string::isUriUnreserved(position))
        {
        ...
        {
            if (internal::string::isAllDigits(position))
            {
                return PositionValidationResult::Valid;
            }
        ...
        {
            if (internal::string::isAllDigits(parts[i]) && i < parts.size() - 1)
            {
                numberNotAtEnd = true;
                break;
            }
        ...
        for (auto part : parts)
        {
            if (!internal::string::isAllDigits(part))
            {
