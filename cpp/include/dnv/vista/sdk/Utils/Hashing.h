...
namespace dnv::vista::sdk::utils
{
	namespace Hash
	{
		...
		VISTA_SDK_CPP_FORCE_INLINE int hashContainer( const Container& container ) noexcept
		{
			...
			{
				seed ^= hash( item ) + constants::algorithm::FNV_OFFSET_BASIS + ( seed << 6 ) + ( seed >> 2 );
			}
		...
		VISTA_SDK_CPP_FORCE_INLINE int hashSequence( const Container& container, const T& finalItem ) noexcept
		{
			...
			seed ^= hash( finalItem ) + constants::algorithm::FNV_OFFSET_BASIS + ( seed << 6 ) + ( seed >> 2 );
		...
		{
			std::size_t seed = 0;
			( ( seed ^= hash( args ) + constants::algorithm::FNV_OFFSET_BASIS + ( seed << 6 ) + ( seed >> 2 ) ), ... );
