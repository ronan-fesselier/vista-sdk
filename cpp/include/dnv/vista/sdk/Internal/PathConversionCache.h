...
namespace dnv::vista::sdk::internal
{
	...
	public:
		...
		VISTA_SDK_CPP_FORCE_INLINE bool tryGetCachedConversion(
			VisVersion sourceVersion, const std::string& pathString, VisVersion targetVersion, std::optional<GmodPath>& result ) const
		{
		...
		VISTA_SDK_CPP_FORCE_INLINE void cacheConversion(
			VisVersion sourceVersion, const std::string& pathString, VisVersion targetVersion, const std::optional<GmodPath>& result )
		{
			PathConversionKey key{ sourceVersion, targetVersion, pathString };
		...
		[[nodiscard]] VISTA_SDK_CPP_FORCE_INLINE CacheStats getStats() const
		{
			...
			return CacheStats{ hits, misses, total > 0 ? static_cast<double>( hits ) / static_cast<double>( total ) : 0.0, m_pathCache.size() };
		}
