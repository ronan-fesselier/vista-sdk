...
TEST( Decimal128Test, DefaultConstruction )
{
	...
	EXPECT_TRUE( Decimal128.isZero() );
	EXPECT_FALSE( Decimal128.isNegative() );
	EXPECT_EQ( Decimal128.scale(), 0 );
	EXPECT_EQ( Decimal128.toString(), "0" );
}
...
TEST( Decimal128Test, IntegerConstruction )
{
	...
	/* Large integers */
	Decimal128 d4( std::numeric_limits<std::int64_t>::max() );
...
{
	/* Simple double */
	Decimal128 d1( 123.456 );
	EXPECT_FALSE( d1.isZero() );
	EXPECT_FALSE( d1.isNegative() );

	/* Negative double */
	Decimal128 d2( -123.456 );
	EXPECT_FALSE( d2.isZero() );
	EXPECT_TRUE( d2.isNegative() );
...
TEST( Decimal128Test, StringConstruction )
{
	...
	Decimal128 d2( "-789.123" );
	EXPECT_TRUE( d2.isNegative() );

	Decimal128 d3( "0" );
	EXPECT_TRUE( d3.isZero() );

	Decimal128 d4( "0.0001" );
	EXPECT_FALSE( d4.isZero() );

	/* Test with many Decimal128 places */
	Decimal128 d5( "123.1234567890123456789" );
...
TEST( Decimal128Test, TryParse )
{
	...
	/* Valid cases */
	EXPECT_TRUE( Decimal128::tryParse( "123.456", result ) );
	EXPECT_EQ( result.toString(), "123.456" );

	EXPECT_TRUE( Decimal128::tryParse( "-789", result ) );
	EXPECT_TRUE( result.isNegative() );

	EXPECT_TRUE( Decimal128::tryParse( "0", result ) );
	EXPECT_TRUE( result.isZero() );

	/* Invalid cases */
	EXPECT_FALSE( Decimal128::tryParse( "", result ) );
	EXPECT_FALSE( Decimal128::tryParse( "abc", result ) );
	EXPECT_FALSE( Decimal128::tryParse( "12.34.56", result ) );
	EXPECT_FALSE( Decimal128::tryParse( "12a34", result ) );
}
...
{
	/* Basic cases */
	EXPECT_EQ( Decimal128( 123 ).toString(), "123" );
	EXPECT_EQ( Decimal128( -456 ).toString(), "-456" );
	EXPECT_EQ( Decimal128( 0 ).toString(), "0" );
...
{
	Decimal128 d1( "123.45" );
	Decimal128 d2( "67.89" );
	Decimal128 result = d1 + d2;
...
TEST( Decimal128Test, Division )
{
	...
	EXPECT_THROW( d1 / Decimal128( 0 ), std::overflow_error );
}
...
{
	Decimal128 d1( "123.45" );
	Decimal128 d2( "123.45" );
	Decimal128 d3( "123.46" );
...
TEST( Decimal128Test, MaxPrecision )
{
	...
	Decimal128 d1;
	EXPECT_TRUE( Decimal128::tryParse( max_precision, d1 ) );
	EXPECT_FALSE( d1.isZero() );

	/* Test exceeding max precision should fail */
	/* 29 Decimal128 places */
	std::string too_precise = "1.23456789012345678901234567890";
	EXPECT_FALSE( Decimal128::tryParse( too_precise, d1 ) );
}

TEST( Decimal128Test, LargeNumbers )
{
	/* Test with large integers */
	Decimal128 d1( std::numeric_limits<std::int64_t>::max() );
	EXPECT_FALSE( d1.isZero() );
	EXPECT_FALSE( d1.isNegative() );

	Decimal128 d2( std::numeric_limits<std::int64_t>::min() );
	EXPECT_FALSE( d2.isZero() );
	EXPECT_TRUE( d2.isNegative() );
}
...
TEST( Decimal128Test, VerySmallNumbers )
{
	...
	Decimal128 d1( "0.0000000000000000000000000001" );
	EXPECT_FALSE( d1.isZero() );
	EXPECT_FALSE( d1.isNegative() );
...
TEST( Decimal128Test, CrossPlatformConsistency )
{
	...
	Decimal128 d1( "123456789.123456789" );
	Decimal128 d2( "987654321.987654321" );

	/* Addition should work consistently */
	Decimal128 sum = d1 + d2;
	EXPECT_FALSE( sum.isZero() );

	/* Multiplication should work consistently */
	Decimal128 product = Decimal128( "123.456" ) * Decimal128( "789.123" );
	EXPECT_FALSE( product.isZero() );

	/* Division should work consistently */
	Decimal128 quotient = Decimal128( "1000" ) / Decimal128( "3" );
...
TEST( Decimal128Test, StressTestLargeOperations )
{
	...
	Decimal128 accumulator( "0" );
	Decimal128 increment( "0.001" );
...
TEST( Decimal128Test, TruncateMethod )
{
	...
	Decimal128 d1( "123.789" );
	Decimal128 truncated = d1.truncate();
	EXPECT_EQ( truncated.toString(), "123" );

	/* Test negative truncation */
	Decimal128 d2( "-123.789" );
	...
	/* Test static method */
	Decimal128 d5( "456.789" );
	Decimal128 staticTruncated = Decimal128::truncate( d5 );
...
TEST( Decimal128Test, FloorMethod )
{
	...
	Decimal128 d3( "123.001" );
	floored = d3.floor();
	EXPECT_EQ( floored.toString(), "123" );

	/* Test negative value close to integer */
	Decimal128 d4( "-123.001" );
	floored = d4.floor();
	EXPECT_EQ( floored.toString(), "-124" );
	...
	/* Test static method */
	Decimal128 d6( "456.789" );
	Decimal128 staticFloored = Decimal128::floor( d6 );
...
TEST( Decimal128Test, CeilingMethod )
{
	...
	Decimal128 d1( "123.123" );
	Decimal128 ceiled = d1.ceiling();
	EXPECT_EQ( ceiled.toString(), "124" );

	/* Test negative values */
	Decimal128 d2( "-123.123" );
	ceiled = d2.ceiling();
	...
	/* Test static method */
	Decimal128 d6( "456.123" );
	Decimal128 staticCeiled = Decimal128::ceiling( d6 );
...
TEST( Decimal128Test, RoundMethod )
{
	...
	/* Test static method */
	Decimal128 d6( "456.7" );
	Decimal128 staticRounded = Decimal128::round( d6 );
...
TEST( Decimal128Test, RoundWithDecimalPlaces )
{
	...
	Decimal128 d1( "123.4567" );

	/* Round to 2 Decimal128 places */
	Decimal128 rounded = d1.round( 2 );
...
TEST( Decimal128Test, AbsMethod )
{
	...
	/* Test static method */
	Decimal128 d5( "-789.123" );
	Decimal128 staticAbs = Decimal128::abs( d5 );
	EXPECT_EQ( staticAbs.toString(), "789.123" );
	EXPECT_FALSE( staticAbs.isNegative() );

	/* Test very small negative value */
	Decimal128 d6( "-0.000000000000000000000000001" );
	absValue = d6.abs();
	EXPECT_FALSE( absValue.isNegative() );
...
TEST( Decimal128Test, MathematicalConsistency )
{
	...
	/* Test that (a + b) - a == b */
	Decimal128 sum = a + b;
	Decimal128 diff = sum - a;
	EXPECT_TRUE( diff == b );

	/* Test that a * b / a == b (within precision limits) */
	Decimal128 product = a * b;
	Decimal128 quotient = product / a;
	Decimal128 difference = quotient - b;

	/* Due to precision, we test that they're very close */
	Decimal128 tolerance( "0.00001" );
...
TEST( Decimal128Test, RoundingConsistency )
{
	...
	Decimal128 value( "123.456789" );

	/* Truncate should always round toward zero */
	Decimal128 truncated = value.truncate();
	EXPECT_EQ( truncated.toString(), "123" );

	/* Floor should always round down */
	Decimal128 floored = value.floor();
	EXPECT_EQ( floored.toString(), "123" );

	/* Ceiling should always round up */
	Decimal128 ceiled = value.ceiling();
	EXPECT_EQ( ceiled.toString(), "124" );

	/* Round should use banker's rounding */
	Decimal128 rounded = value.round();
	EXPECT_EQ( rounded.toString(), "123" );

	/* Test with negative values */
	Decimal128 negValue( "-123.456789" );
...
TEST( Decimal128Test, PrecisionPreservation )
{
	...
	/* Test with many Decimal128 places */
	Decimal128 precise1( "0.1234567890123456789012345678" );
	Decimal128 precise2( "0.0000000000000000000000000001" );
	Decimal128 preciseSum = precise1 + precise2;
...
TEST( Decimal128Test, OverflowHandling )
{
	...
	try
	{
		...
		Decimal128 large1( "99999999999999999999999999.99" );
		Decimal128 large2( "1.01" );

		/* Operations should either succeed or throw appropriately */
		Decimal128 result = large1 * large2;
...
TEST( Decimal128Test, InvalidInputHandling )
{
	...
	EXPECT_FALSE( Decimal128::tryParse( "12.34.56", result ) );
	EXPECT_FALSE( Decimal128::tryParse( "12a34", result ) );
	EXPECT_FALSE( Decimal128::tryParse( "+-123", result ) );
	EXPECT_FALSE( Decimal128::tryParse( "123..", result ) );
	EXPECT_FALSE( Decimal128::tryParse( ".123.", result ) );
	...
	EXPECT_FALSE( Decimal128::tryParse( tooLong, result ) );
}
