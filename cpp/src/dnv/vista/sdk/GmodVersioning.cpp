...
namespace dnv::vista::sdk
{
	namespace
	{
		struct QualifyingNodePair
		{
			...
			QualifyingNodePair( const GmodNode* source, GmodNode target )
				: sourceNode( source ), targetNode( std::move( target ) ) {}
		};
	}
		...
		VisVersion sourceVersion, const GmodPath& sourcePath, VisVersion targetVersion ) const
	{
		...
		while ( enumerator.next() )
		{
			...
			qualifyingNodes.emplace_back( sourceNode, std::move( targetNode.value() ) );
		}
