...
namespace dnv::vista::sdk::datatypes
{
		  ...
		  m_flags{ 0 }
	{
		...
		while ( currentScale < 15 && currentScale < constants::decimal128::MAXIMUM_PLACES &&
	...
	std::string Decimal128::toString() const
	{
		...
		else
		{
			...
			{
				digits[digitCount++] = static_cast<char>( constants::decimal128::DIGIT_OFFSET + ( mantissa.m_value % constants::decimal128::BASE ) );
		...
		else
		{
			...
			while ( !mantissa.isZero() && digitCount < digits.size() )
			{
				...
				digits[digitCount++] = static_cast<char>( constants::decimal128::DIGIT_OFFSET + ( mantissa.m_lower64bits % constants::decimal128::BASE ) );
	...
	Decimal128 Decimal128::round( std::int32_t decimalsPlacesCount ) const noexcept
	{
		...
		result.m_flags = ( result.m_flags & ~constants::decimal128::SCALE_MASK ) |
	...
	void Decimal128::normalize() noexcept
	{
		...
		while ( scale() > 0 && ( getMantissa() % Int128( constants::decimal128::BASE ) ) == Int128( 0 ) )
		{
			...
			std::uint8_t currentScale = scale();
			m_flags = ( m_flags & ~constants::decimal128::SCALE_MASK ) | ( static_cast<std::uint32_t>( currentScale - 1U ) << constants::decimal128::SCALE_SHIFT );
		}
	}
	...
	std::pair<Int128, Int128> Decimal128::alignScale( const Decimal128& other ) const
	{
		...
		if ( leftScale < rightScale )
		{
			...
			if ( scaleDiff < constants::decimal128::POWER_TABLE_SIZE && constants::decimal128::POWERS_OF_10[static_cast<size_t>( scaleDiff )] != 0 )
			{
