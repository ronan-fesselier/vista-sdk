...
namespace dnv::vista::sdk::benchmarks
{
	...
	public:
		...
		std::vector<Int128> valuesInt128;
	...
	static void BM_Int128_ConstructionFromUint64( benchmark::State& state )
	{
		...
		for ( auto _ : state )
		{
			Int128 value( g_benchData.values64[index % BenchmarkData::DATASET_SIZE] );
			benchmark::DoNotOptimize( value );
			++index;
		}
		state.SetItemsProcessed( state.iterations() );
	...
	static void BM_Int128_ConstructionFromInt64( benchmark::State& state )
	{
		...
		for ( auto _ : state )
		{
			Int128 value( static_cast<std::int64_t>( g_benchData.values64[index % BenchmarkData::DATASET_SIZE] ) );
			benchmark::DoNotOptimize( value );
			++index;
		}
		state.SetItemsProcessed( state.iterations() );
	...
	static void BM_Int128_FastPath64Bit( benchmark::State& state )
	{
		...
		for ( auto _ : state )
		{
			Int128 a( g_benchData.values64[index % BenchmarkData::DATASET_SIZE] );
			Int128 b( g_benchData.values64[( index + 1 ) % BenchmarkData::DATASET_SIZE] );
