...
namespace dnv::vista::sdk::benchmarks
{
	...
	private:
		void generateRandomDecimals()
		{
			...
			{
				m_randomDecimals.emplace_back( Decimal128( dist( gen ) ) );
			}
		}
		...
		void generateTypicalBusinessValues()
		{
			...
			m_businessValues = {
				Decimal128( 0.01 ), Decimal128( 0.99 ), Decimal128( 1.00 ), Decimal128( 9.99 ),
				Decimal128( 10.50 ), Decimal128( 99.95 ), Decimal128( 100.00 ), Decimal128( 999.99 ),
				Decimal128( 1000.00 ), Decimal128( 9999.99 ), Decimal128( 12345.67 ), Decimal128( 100000.00 ),
				Decimal128( -0.01 ), Decimal128( -9.99 ), Decimal128( -100.00 ), Decimal128( -1000.00 ) };
		}

		void generateStringData()
		{
			m_stringValues = {
				"0", "1", "-1", "0.1", "-0.1", "123.456", "-123.456",
				"999999.999999", "-999999.999999", "1000000000.123456789",
				"0.000000001", "12345678901234567890.123456789",
				"79228162514264337593543950335", "-79228162514264337593543950335" };

			m_doubleValues = {
				0.0, 1.0, -1.0, 0.1, -0.1, 123.456, -123.456,
				999999.999999, -999999.999999, 1000000000.123456789,
				0.000000001, 1.23e15, -1.23e15, 1.79e308, -1.79e308 };
		}
	};
	...
	BENCHMARK_F( DecimalBenchmarkFixture, Construction_FromDouble )( benchmark::State& state )
	{
		...
		for ( auto _ : state )
		{
			Decimal128 d( m_doubleValues[index % m_doubleValues.size()] );
			benchmark::DoNotOptimize( d );
			++index;
		}
		state.SetItemsProcessed( state.iterations() );
	...
	BENCHMARK_F( DecimalBenchmarkFixture, Construction_FromString )( benchmark::State& state )
	{
		...
		for ( auto _ : state )
		{
			Decimal128 d( m_stringValues[index % m_stringValues.size()] );
			benchmark::DoNotOptimize( d );
			++index;
		}
		state.SetItemsProcessed( state.iterations() );
	...
	{
		const Decimal128 source( "123.456789" );
		for ( auto _ : state )
		{
			Decimal128 d( source );
			benchmark::DoNotOptimize( d );
	...
	{
		Decimal128 accumulator = Decimal128::zero();
		size_t index = 0;
		for ( auto _ : state )
		{
	...
	BENCHMARK_F( DecimalBenchmarkFixture, Math_Round )( benchmark::State& state )
	{
		...
		{
			const auto& value = m_randomDecimals[index % m_randomDecimals.size()];
			Decimal128 result = value.round( 2 );
			benchmark::DoNotOptimize( result );
			++index;
		}
		state.SetItemsProcessed( state.iterations() );
	...
	BENCHMARK_F( DecimalBenchmarkFixture, Math_Truncate )( benchmark::State& state )
	{
		...
		{
			const auto& value = m_randomDecimals[index % m_randomDecimals.size()];
			Decimal128 result = value.truncate();
			benchmark::DoNotOptimize( result );
			++index;
		}
		state.SetItemsProcessed( state.iterations() );
	...
	BENCHMARK_F( DecimalBenchmarkFixture, Math_Ceiling )( benchmark::State& state )
	{
		...
		{
			const auto& value = m_randomDecimals[index % m_randomDecimals.size()];
			Decimal128 result = value.ceiling();
			benchmark::DoNotOptimize( result );
			++index;
		}
		state.SetItemsProcessed( state.iterations() );
	...
	BENCHMARK_F( DecimalBenchmarkFixture, Parsing_TryParse_Valid )( benchmark::State& state )
	{
		...
		for ( auto _ : state )
		{
			...
			Decimal128 result;
			bool success = Decimal128::tryParse( str, result );
			benchmark::DoNotOptimize( result );
	...
	BENCHMARK_F( DecimalBenchmarkFixture, Parsing_TryParse_Invalid )( benchmark::State& state )
	{
		const std::vector<std::string> invalidStrings = {
			"", "abc", "12.34.56", "1.2.3", "not_a_number", "1e10", "inf", "nan" };
	...
	BENCHMARK_F( DecimalBenchmarkFixture, Scenario_FinancialCalculation )( benchmark::State& state )
	{
		...
		const Decimal128 principal( "10000.00" );
		const Decimal128 rate( "0.05" );
		const Decimal128 one = Decimal128::one();

		for ( auto _ : state )
		{
			Decimal128 factor = one + rate;
			Decimal128 result = principal;
	...
	BENCHMARK_F( DecimalBenchmarkFixture, Scenario_SummationAccuracy )( benchmark::State& state )
	{
		...
		const Decimal128 smallValue( "0.01" );
