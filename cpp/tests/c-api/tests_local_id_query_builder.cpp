...
TEST_SUITE("c-api::local_id_query_builder")
{
    ...
    TEST_CASE("null builder handle - accessors fail gracefully")
    {
        ...
        CHECK(dnv_vista_sdk_local_id_query_builder_build(nullptr) == nullptr);
        CHECK(dnv_vista_sdk_local_id_query_builder_tags_builder(nullptr) == nullptr);
    }

    TEST_CASE("dnv_vista_sdk_local_id_query_builder_tags_builder - returns null when no tags configured")
    {
        auto* builder = dnv_vista_sdk_local_id_query_builder_create();
        REQUIRE(builder != nullptr);

        CHECK(dnv_vista_sdk_local_id_query_builder_tags_builder(builder) == nullptr);

        dnv_vista_sdk_local_id_query_builder_free(builder);
    }

    TEST_CASE("dnv_vista_sdk_local_id_query_builder_with_primary_item_nodes_builder - matches by node")
    {
        const auto* vis = dnv_vista_sdk_vis_instance();
        const auto* gmod = dnv_vista_sdk_vis_gmod(vis, "3-4a");
        REQUIRE(gmod != nullptr);

        const auto* nodeC101 = dnv_vista_sdk_gmod_get_node(gmod, "C101");
        REQUIRE(nodeC101 != nullptr);

        auto* nb0 = dnv_vista_sdk_gmod_path_query_builder_create();
        auto* nb1 = dnv_vista_sdk_gmod_path_query_builder_with_node_all_locations(nb0, nodeC101, 1);
        REQUIRE(nb1 != nullptr);

        auto* b0 = dnv_vista_sdk_local_id_query_builder_create();
        auto* b1 = dnv_vista_sdk_local_id_query_builder_with_primary_item_nodes_builder(b0, nb1);
        REQUIRE(b1 != nullptr);
        auto* query = dnv_vista_sdk_local_id_query_builder_build(b1);
        REQUIRE(query != nullptr);

        auto* matching = dnv_vista_sdk_local_id_from_string("/dnv-v2/vis-3-4a/411.1/C101.31/meta/qty-power");
        REQUIRE(matching != nullptr);
        CHECK(dnv_vista_sdk_local_id_query_match(query, matching) == 1);

        auto* nonMatching = dnv_vista_sdk_local_id_from_string("/dnv-v2/vis-3-4a/411.1/C102.31/meta/qty-power");
        REQUIRE(nonMatching != nullptr);
        CHECK(dnv_vista_sdk_local_id_query_match(query, nonMatching) == 0);

        dnv_vista_sdk_local_id_free(matching);
        dnv_vista_sdk_local_id_free(nonMatching);
        dnv_vista_sdk_local_id_query_free(query);
        dnv_vista_sdk_local_id_query_builder_free(b0);
        dnv_vista_sdk_local_id_query_builder_free(b1);
        dnv_vista_sdk_gmod_path_query_builder_free(nb0);
        dnv_vista_sdk_gmod_path_query_builder_free(nb1);
    }

    TEST_CASE("dnv_vista_sdk_local_id_query_builder_with_primary_item_nodes_builder - null arguments return null")
    {
        const auto* vis = dnv_vista_sdk_vis_instance();
        const auto* gmod = dnv_vista_sdk_vis_gmod(vis, "3-4a");
        const auto* nodeC101 = dnv_vista_sdk_gmod_get_node(gmod, "C101");
        auto* nb = dnv_vista_sdk_gmod_path_query_builder_create();
        auto* nb1 = dnv_vista_sdk_gmod_path_query_builder_with_node_all_locations(nb, nodeC101, 1);
        auto* builder = dnv_vista_sdk_local_id_query_builder_create();

        CHECK(dnv_vista_sdk_local_id_query_builder_with_primary_item_nodes_builder(nullptr, nullptr) == nullptr);
        CHECK(dnv_vista_sdk_local_id_query_builder_with_primary_item_nodes_builder(builder, nullptr) == nullptr);
        CHECK(dnv_vista_sdk_local_id_query_builder_with_primary_item_nodes_builder(nullptr, nb1) == nullptr);

        dnv_vista_sdk_local_id_query_builder_free(builder);
        dnv_vista_sdk_gmod_path_query_builder_free(nb);
        dnv_vista_sdk_gmod_path_query_builder_free(nb1);
    }

    TEST_CASE("dnv_vista_sdk_local_id_query_builder_with_primary_item_nodes_builder - Path-variant handle rejected")
    {
        auto* basePath = dnv_vista_sdk_gmod_path_from_short_path_version("433.1-S/C322.91/S205", "3-7a");
        REQUIRE(basePath != nullptr);

        auto* pb = dnv_vista_sdk_gmod_path_query_builder_from(basePath);
        REQUIRE(pb != nullptr);

        auto* builder = dnv_vista_sdk_local_id_query_builder_create();
        CHECK(dnv_vista_sdk_local_id_query_builder_with_primary_item_nodes_builder(builder, pb) == nullptr);
        CHECK(std::string_view{ dnv_vista_sdk_last_error_message() }.size() > 0);

        dnv_vista_sdk_gmod_path_free(basePath);
        dnv_vista_sdk_local_id_query_builder_free(builder);
        dnv_vista_sdk_gmod_path_query_builder_free(pb);
    }

    TEST_CASE("dnv_vista_sdk_local_id_query_builder_with_primary_item_path_builder - matches with configured path")
    {
        auto* basePath = dnv_vista_sdk_gmod_path_from_short_path_version("433.1-S/C322.91/S205", "3-7a");
        REQUIRE(basePath != nullptr);

        auto* pb0 = dnv_vista_sdk_gmod_path_query_builder_from(basePath);
        auto* pb1 = dnv_vista_sdk_gmod_path_query_builder_path_with_node_all_locations(pb0, "433.1", 1);
        REQUIRE(pb1 != nullptr);

        auto* b0 = dnv_vista_sdk_local_id_query_builder_create();
        auto* b1 = dnv_vista_sdk_local_id_query_builder_with_primary_item_path_builder(b0, pb1);
        REQUIRE(b1 != nullptr);
        auto* query = dnv_vista_sdk_local_id_query_builder_build(b1);
        REQUIRE(query != nullptr);

        auto* matching = dnv_vista_sdk_local_id_from_string(
    ...
    TEST_CASE("dnv_vista_sdk_local_id_query_builder_with_primary_item_path_builder - Nodes-variant handle rejected")
    {
        const auto* vis = dnv_vista_sdk_vis_instance();
        const auto* gmod = dnv_vista_sdk_vis_gmod(vis, "3-4a");
        REQUIRE(gmod != nullptr);
        ...
        auto* builder = dnv_vista_sdk_local_id_query_builder_create();
        CHECK(dnv_vista_sdk_local_id_query_builder_with_primary_item_path_builder(builder, nb1) == nullptr);
        CHECK(std::string_view{ dnv_vista_sdk_last_error_message() }.size() > 0);
    ...
    TEST_CASE("dnv_vista_sdk_local_id_query_builder_with_secondary_item_nodes_builder - matches by node")
    {
        const auto* vis = dnv_vista_sdk_vis_instance();
        const auto* gmod = dnv_vista_sdk_vis_gmod(vis, "3-4a");
        REQUIRE(gmod != nullptr);

        auto* primaryPath = dnv_vista_sdk_gmod_path_from_short_path_version("411.1/C101.31", "3-4a");
        REQUIRE(primaryPath != nullptr);

        const auto* node652 = dnv_vista_sdk_gmod_get_node(gmod, "652.1i");
        REQUIRE(node652 != nullptr);

        auto* nb0 = dnv_vista_sdk_gmod_path_query_builder_create();
        auto* nb1 = dnv_vista_sdk_gmod_path_query_builder_with_node_all_locations(nb0, node652, 1);
        REQUIRE(nb1 != nullptr);

        auto* b0 = dnv_vista_sdk_local_id_query_builder_create();
        auto* b1 = dnv_vista_sdk_local_id_query_builder_with_primary_item(b0, primaryPath);
        REQUIRE(b1 != nullptr);
        auto* b2 = dnv_vista_sdk_local_id_query_builder_with_secondary_item_nodes_builder(b1, nb1);
        REQUIRE(b2 != nullptr);
        auto* query = dnv_vista_sdk_local_id_query_builder_build(b2);
        REQUIRE(query != nullptr);

        auto* matching =
            dnv_vista_sdk_local_id_from_string("/dnv-v2/vis-3-4a/411.1/C101.31/sec/652.1i-1P/meta/qty-power");
        REQUIRE(matching != nullptr);
        CHECK(dnv_vista_sdk_local_id_query_match(query, matching) == 1);
        ...
        dnv_vista_sdk_local_id_query_builder_free(b0);
        dnv_vista_sdk_local_id_query_builder_free(b1);
        dnv_vista_sdk_local_id_query_builder_free(b2);
        dnv_vista_sdk_gmod_path_query_builder_free(nb0);
        dnv_vista_sdk_gmod_path_query_builder_free(nb1);
    }

    TEST_CASE("dnv_vista_sdk_local_id_query_builder_with_secondary_item_nodes_builder - Path-variant handle rejected")
    {
        auto* basePath = dnv_vista_sdk_gmod_path_from_short_path_version("433.1-S/C322.91/S205", "3-7a");
        REQUIRE(basePath != nullptr);
        ...
        auto* builder = dnv_vista_sdk_local_id_query_builder_create();
        CHECK(dnv_vista_sdk_local_id_query_builder_with_secondary_item_nodes_builder(builder, pb) == nullptr);
        CHECK(std::string_view{ dnv_vista_sdk_last_error_message() }.size() > 0);
    ...
    TEST_CASE("dnv_vista_sdk_local_id_query_builder_with_secondary_item_path_builder - matches with configured path")
    {
        auto* primaryPath = dnv_vista_sdk_gmod_path_from_short_path_version("411.1/C101.31", "3-4a");
        REQUIRE(primaryPath != nullptr);
        auto* secondaryPath = dnv_vista_sdk_gmod_path_from_short_path_version("652.1i-1P", "3-4a");
        REQUIRE(secondaryPath != nullptr);

        auto* pb0 = dnv_vista_sdk_gmod_path_query_builder_from(secondaryPath);
        auto* pb1 = dnv_vista_sdk_gmod_path_query_builder_without_locations(pb0);
        REQUIRE(pb1 != nullptr);
        ...
        auto* b1 = dnv_vista_sdk_local_id_query_builder_with_primary_item(b0, primaryPath);
        REQUIRE(b1 != nullptr);
        auto* b2 = dnv_vista_sdk_local_id_query_builder_with_secondary_item_path_builder(b1, pb1);
        REQUIRE(b2 != nullptr);
        auto* query = dnv_vista_sdk_local_id_query_builder_build(b2);
        REQUIRE(query != nullptr);
        ...
        dnv_vista_sdk_local_id_free(matching);
        dnv_vista_sdk_gmod_path_free(primaryPath);
        dnv_vista_sdk_gmod_path_free(secondaryPath);
        dnv_vista_sdk_local_id_query_free(query);
        dnv_vista_sdk_local_id_query_builder_free(b0);
        dnv_vista_sdk_local_id_query_builder_free(b1);
        dnv_vista_sdk_local_id_query_builder_free(b2);
    ...
    TEST_CASE("dnv_vista_sdk_local_id_query_builder_with_secondary_item_path_builder - Nodes-variant handle rejected")
    {
        const auto* vis = dnv_vista_sdk_vis_instance();
        const auto* gmod = dnv_vista_sdk_vis_gmod(vis, "3-4a");
        REQUIRE(gmod != nullptr);
        ...
        auto* builder = dnv_vista_sdk_local_id_query_builder_create();
        CHECK(dnv_vista_sdk_local_id_query_builder_with_secondary_item_path_builder(builder, nb1) == nullptr);
        CHECK(std::string_view{ dnv_vista_sdk_last_error_message() }.size() > 0);
    ...
    TEST_CASE("dnv_vista_sdk_local_id_query_builder_tags_builder - chained calls accumulate tags")
    {
        auto* tb0 = dnv_vista_sdk_metadata_tags_query_builder_create();
        auto* tb1 =
            dnv_vista_sdk_metadata_tags_query_builder_with_tag(tb0, DNV_VISTA_SDK_CODEBOOK_NAME_CONTENT, "sea.water");
        REQUIRE(tb1 != nullptr);
        auto* firstTagsQuery = dnv_vista_sdk_metadata_tags_query_builder_build(tb1);
        REQUIRE(firstTagsQuery != nullptr);

        auto* b0 = dnv_vista_sdk_local_id_query_builder_create();
        auto* b1 = dnv_vista_sdk_local_id_query_builder_with_tags(b0, firstTagsQuery);
        REQUIRE(b1 != nullptr);

        const auto* existingTagsBuilder = dnv_vista_sdk_local_id_query_builder_tags_builder(b1);
        REQUIRE(existingTagsBuilder != nullptr);

        auto* tb2 = dnv_vista_sdk_metadata_tags_query_builder_with_tag(
            existingTagsBuilder, DNV_VISTA_SDK_CODEBOOK_NAME_STATE, "opened");
        REQUIRE(tb2 != nullptr);
        auto* accumulatedTagsQuery = dnv_vista_sdk_metadata_tags_query_builder_build(tb2);
        REQUIRE(accumulatedTagsQuery != nullptr);

        auto* b2 = dnv_vista_sdk_local_id_query_builder_with_tags(b1, accumulatedTagsQuery);
        REQUIRE(b2 != nullptr);
        auto* query = dnv_vista_sdk_local_id_query_builder_build(b2);
        REQUIRE(query != nullptr);

        auto* localIdBothTags = dnv_vista_sdk_local_id_from_string(
            "/dnv-v2/vis-3-4a/652.31/S90.3/S61/sec/652.1i-1P/meta/cnt-sea.water/state-opened");
        REQUIRE(localIdBothTags != nullptr);
        CHECK(dnv_vista_sdk_local_id_query_match(query, localIdBothTags) == 1);

        auto* localIdOneTag =
            dnv_vista_sdk_local_id_from_string("/dnv-v2/vis-3-4a/652.31/S90.3/S61/sec/652.1i-1P/meta/cnt-sea.water");
        REQUIRE(localIdOneTag != nullptr);
        CHECK(dnv_vista_sdk_local_id_query_match(query, localIdOneTag) == 0);

        dnv_vista_sdk_local_id_free(localIdBothTags);
        dnv_vista_sdk_local_id_free(localIdOneTag);
        dnv_vista_sdk_local_id_query_free(query);
        dnv_vista_sdk_local_id_query_builder_free(b0);
        dnv_vista_sdk_local_id_query_builder_free(b1);
        dnv_vista_sdk_local_id_query_builder_free(b2);
        dnv_vista_sdk_metadata_tags_query_free(firstTagsQuery);
        dnv_vista_sdk_metadata_tags_query_free(accumulatedTagsQuery);
        dnv_vista_sdk_metadata_tags_query_builder_free(tb0);
        dnv_vista_sdk_metadata_tags_query_builder_free(tb1);
        dnv_vista_sdk_metadata_tags_query_builder_free(tb2);
    }
}
