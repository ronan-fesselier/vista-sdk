...
TEST_SUITE("c-api::gmod_node")
{
    ...
    TEST_CASE("dnv_vista_sdk_gmod_node_equals - same node is equal to itself")
    {
        const auto* vis = dnv_vista_sdk_vis_instance();
        const auto* gmod = dnv_vista_sdk_vis_gmod(vis, "3-11a");
        const auto* root = dnv_vista_sdk_gmod_root_node(gmod);

        CHECK(dnv_vista_sdk_gmod_node_equals(root, root) == 1);
    }

    TEST_CASE("dnv_vista_sdk_gmod_node_equals - different nodes are not equal")
    {
        const auto* vis = dnv_vista_sdk_vis_instance();
        const auto* gmod = dnv_vista_sdk_vis_gmod(vis, "3-11a");
        const auto* root = dnv_vista_sdk_gmod_root_node(gmod);
        ...
        CHECK(dnv_vista_sdk_gmod_node_equals(root, child) == 0);
    }

    TEST_CASE("dnv_vista_sdk_gmod_node_equals - null returns 0")
    {
        const auto* vis = dnv_vista_sdk_vis_instance();
        const auto* gmod = dnv_vista_sdk_vis_gmod(vis, "3-11a");
        const auto* root = dnv_vista_sdk_gmod_root_node(gmod);

        CHECK(dnv_vista_sdk_gmod_node_equals(nullptr, root) == 0);
        CHECK(dnv_vista_sdk_gmod_node_equals(root, nullptr) == 0);
        CHECK(dnv_vista_sdk_gmod_node_equals(nullptr, nullptr) == 0);
    }
}
