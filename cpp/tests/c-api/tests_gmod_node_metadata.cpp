...
TEST_SUITE("c-api::gmod_node_metadata")
{
    ...
    TEST_CASE("dnv_vista_sdk_gmod_node_metadata_equals - same metadata is equal to itself")
    {
        const auto* vis = dnv_vista_sdk_vis_instance();
        const auto* gmod = dnv_vista_sdk_vis_gmod(vis, "3-11a");
        const auto* root = dnv_vista_sdk_gmod_root_node(gmod);
        ...
        CHECK(dnv_vista_sdk_gmod_node_metadata_equals(metadata, metadata) == 1);
    }

    TEST_CASE("dnv_vista_sdk_gmod_node_metadata_equals - different nodes have different metadata")
    {
        const auto* vis = dnv_vista_sdk_vis_instance();
        const auto* gmod = dnv_vista_sdk_vis_gmod(vis, "3-11a");
        const auto* root = dnv_vista_sdk_gmod_root_node(gmod);
        REQUIRE(dnv_vista_sdk_gmod_node_child_count(root) > 0);
        const auto* child = dnv_vista_sdk_gmod_node_child_at(root, 0);

        const auto* meta_root = dnv_vista_sdk_gmod_node_metadata(root);
        const auto* meta_child = dnv_vista_sdk_gmod_node_metadata(child);

        CHECK(dnv_vista_sdk_gmod_node_metadata_equals(meta_root, meta_child) == 0);
    }

    TEST_CASE("dnv_vista_sdk_gmod_node_metadata_equals - null returns 0")
    {
        const auto* vis = dnv_vista_sdk_vis_instance();
        const auto* gmod = dnv_vista_sdk_vis_gmod(vis, "3-11a");
        const auto* root = dnv_vista_sdk_gmod_root_node(gmod);
        ...
        CHECK(dnv_vista_sdk_gmod_node_metadata_equals(nullptr, metadata) == 0);
        CHECK(dnv_vista_sdk_gmod_node_metadata_equals(metadata, nullptr) == 0);
        CHECK(dnv_vista_sdk_gmod_node_metadata_equals(nullptr, nullptr) == 0);
    }
